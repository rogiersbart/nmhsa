# NOTE
#
# - print calls need "flush = True" to avoid that all messages are only returned
#   to R when the python function call has completely finished
# - note this is not used here anymore, in favour of the {rui} functions,
#   exposed in python as a dictionary with dot access, by the R wrapper

import numpy as np
from scipy import ndimage as ndi


def wrapper(ti, mode="NMH-SA"):
    """
    This function is a wrapper to reconstruct the images from the paper, the
    code returns a reconstructed image) to run the simulation  using the
    hierachical approach only type 'H-SA'
    """
    if mode == "H-SA":
        sg = NMH_SA(
            ti,
        ...
        splitted_cementimage = hole_removement(
            ti, 3, 2, 5
        )  # Image which gives indication about merged sample
        ...
        splitted_merged = phasesplit(
            splitted_merged, 1, 3, [5, 1]
        )  # relabeling all Pore Nodes which are bigger than 3
        splitted_merged = phasesplit(
            splitted_merged, 2, 3, [6, 2]
        )  # relabeling all Portlandite Nodtes which are bigger than 3
        splitted_merged = phasesplit(splitted_merged, 3, 3, [7, 3])
        splitted_merged = phasesplit(splitted_merged, 7, 15, [8, 7])
        sg = NMH_SA(
            splitted_merged,
            order=[8, 7, 5, 6, 3, 1, 2, 4],
            gridlevel=[3, 2, 2, 2, 1, 1, 1],
            length=[30, 30, 30, 30, 30, 30, 30],
            nt=[10001, 10001, 10001, 10001, 10001, 10001, 10001],
            nps=30,
            nstop=300,
            acc=1 * 10**-6,
            sgd=[],
            lam=0.98,
            nrdir=4,
            mergephasein=[[8, 7, 3], [1, 5], [6, 2], [4]],
            mergephaseto=[3, 1, 2, 4],
            targetm=[[3, 4], [1, 4], [2, 4]],
            lengthm=[40, 40, 40],
            ntm=[15001, 15001, 15001],
            split_target=[5],
            ti_2=splitted_cementimage,
            lengthsplit=[40],
            ntsplit=[10001],
            split_targeto=[[5, 3]],
            rename_target=[[2, 3]],
        ...
        sg = NMH_SA2Dto3D(
            ...
            length=[30, 30, 30, 30, 30, 30, 30],
            nps=30,
            nt=[150001, 150001, 200001, 200001, 200001, 200001, 200001, 150001],
            nstop=500,
            acc=1 * 10**-6,
            sgd=[100, 100, 100],
            lam=0.9,
        ...
        rui.alert("This is not a valid choice for the mode")
        rui.stop("Issue with wrapper mode argument")
    return sg
...
def NMH_SA(
    ...
    nps= [integer] number of swaps used to determine intital T0
    ...
    if len(ordernew) > 2:
        ...
        for i in range(1, len(ordernew) - 1):
            ...
            if gridlevel[i] == 5:
                ...
                sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                    tic, ordernew[i], simout[8], pointsdetermined, sgdc
                )
                ...
                sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                    tic, ordernew[i], simout[6], pointsdetermined, sgdc
                )
            elif gridlevel[i] == 3:
                tic = gridcoarsening_4(ti, tid, np.append(importantpoints, 0))[0]
                sgdc = np.array(np.shape(simout[4]))
                sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                    tic, ordernew[i], simout[4], pointsdetermined, sgdc
                )
            elif gridlevel[i] == 2:
                tic = gridcoarsening(ti, tid, np.append(importantpoints, 0))[0]
                sgdc = np.array(np.shape(simout[2]))
                sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                    tic, ordernew[i], simout[2], pointsdetermined, sgdc
                )

            elif gridlevel[i] == 1:
                if type(simout) == tuple:
                    sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                        ti, ordernew[i], simout[0], pointsdetermined, sgd
                    )
                else:
                    sgini, ti, pointsdetermined, nfase = Simgrid_mf(
                        ti, ordernew[i], simout, pointsdetermined, sgd
                    )

            ti = np.copy(tinew)
            mask = np.reshape(
                np.in1d(np.reshape(ti, -1), pointsdetermined), np.shape(ti)
            )
            ti = ti * mask
            target = np.copy(nfase)
            simout = siman_s_l_grf_mf(
                ti,
                sgini,
                target,
                length[i],
    ...
    if merging == 1:
        ...
        if split == 1:
            ...
            sg0, ti2, target_new, rename_targetnew = splitrelable(
                ti2, sg0, megephasto_old1, split_targeto[0], rename_target[0]
            )
            targetphase = target_new
            sg0 = np.reshape(sg0, -1)
            npoins = (
                np.count_nonzero(ti2 == targetphase[0]) * np.size(sg0) / np.size(ti2)
            )
            npoins = int(npoins)
            ...
            if rename_target != []:
                ...
                # rui.inform(rename_targetnew,'h',targetphase)
        ...
        if targetm_new != []:
            for i in range(len(targetm)):
                nfase = targetm_new[i]
                # rui.inform('merging of phase',i+1)
                sg0 = siman_s_l_grf_mf(
                    ti,
                    sg0,
                    nfase,
        ...
        if split == 1:
            ...
            for i in range(len(split_target)):
                ...
                sg0 = np.reshape(sg0, -1)
                npoins = (
                    np.count_nonzero(ti2 == targetphase[0])
                    * np.size(sg0)
                    / np.size(ti2)
                )
                npoins = int(npoins)
...
def hole_removement(ti, surrounding, phasetoremove, newlabel):
    """
    Utility to remove isolated particles within a bigger structure, just looks
    if a particle of phase to remove touches, phasetofill
    ti= 2D input structure
    phasetofill= label of the phase in which the isolated particles are located
    and to which they should be switched after
    phasetoremove= label of the phase from which the isolated particles should
    be removed
...
def NMH_SA2Dto3D(
    ...
    nfase = [pointsdetermined[-1], 0]
    target = np.copy(nfase)
    # rui.inform("target", target)
    if np.size(sgd) == 0:
        sgd = np.shape(ti)
        sgd = np.array(sgd)
    simout = siman_l_s_grf13D(
        ti,
        target,
        length[0],
        nps,
    ...
    for i in range(1, len(ordernew) - 1):
        ...
        importantpoints = np.append(pointsdetermined, ordernew[i])
        ti = np.copy(tinew)
        # rui.inform("phase",i,importantpoints)
...
def label_fct(mt, target=1, Neighbors=4, ret=0):
    ...
    output  if ret =0 only the labeled array is returned,
            else also ot the number of clusters is returned
...
def resh_diag2(mt1):
    x, y = np.shape(mt1)
    s = 0
    row = np.zeros(np.size(mt1), dtype=int)
    col = np.zeros(np.size(mt1), dtype=int)
...
def borderoptsf(sg, sgd, nfase):
    ...
    mt_2 = np.fabs(
        (
            4
            - (mt_1[:-2, 1:-1] == mt_1[1:-1, 1:-1])
            - (mt_1[2:, 1:-1] == mt_1[1:-1, 1:-1])
            - (mt_1[1:-1, 2:] == mt_1[1:-1, 1:-1])
            - (mt_1[1:-1, :-2] == mt_1[1:-1, 1:-1])
        )
    )
...
def borderoptadoptsf(sg, bord, x1, x2, y1, y2, sgd, nfase):
    ...
    if x1 == 0:
        if y1 == 0:
            ...
            bord[0:2, y1 - 1 : y1 + 2] = borderoptsf(
                sg[0:3, y1 - 2 : y1 + 3], [3, 5], nfase
            )[0:2, 1:-1]
    elif x1 == 1:
        if y1 == 0:
            ...
            bord[0:3, y1 - 1 : y1 + 2] = borderoptsf(
                sg[0:4, y1 - 2 : y1 + 3], [4, 5], nfase
            )[0:3, 1:-1]
    elif x1 == X - 2:
        if y1 == 0:
            ...
            bord[-3:, y1 - 1 : y1 + 2] = borderoptsf(
                sg[-4:, y1 - 2 : y1 + 3], [4, 5], nfase
            )[1:, 1:-1]
    elif x1 == X - 1:
        if y1 == 0:
            ...
            bord[-2:, y1 - 1 : y1 + 2] = borderoptsf(
                sg[-3:, y1 - 2 : y1 + 3], [3, 5], nfase
            )[1:, 1:-1]
    else:
        if y1 == 0:
            bord[x1 - 1 : x1 + 2, 0:2] = borderoptsf(
                sg[x1 - 2 : x1 + 3, 0:3], [5, 3], nfase
            )[1:-1, 0:2]
        elif y1 == 1:
            bord[x1 - 1 : x1 + 2, 0:3] = borderoptsf(
                sg[x1 - 2 : x1 + 3, 0:4], [5, 4], nfase
            )[1:-1, 0:3]
        elif y1 == Y - 2:
            bord[x1 - 1 : x1 + 2, -3:] = borderoptsf(
                sg[x1 - 2 : x1 + 3, -4:], [5, 4], nfase
            )[1:-1, 1:]
        elif y1 == Y - 1:
            bord[x1 - 1 : x1 + 2, -2:] = borderoptsf(
                sg[x1 - 2 : x1 + 3, -3:], [5, 3], nfase
            )[1:-1, 1:]
        else:
            bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2] = borderoptsf(
                sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3], [5, 5], nfase
            )[1:-1, 1:-1]
    if x2 == 0:
        if y2 == 0:
            bord[0:2, 0:2] = borderoptsf(sg[0:3, 0:3], [3, 3], nfase)[0:2, 0:2]
            ...
            bord[0:2, y2 - 1 : y2 + 2] = borderoptsf(
                sg[0:3, y2 - 2 : y2 + 3], [3, 5], nfase
            )[0:2, 1:-1]
    elif x2 == 1:
        if y2 == 0:
            bord[0:3, 0:2] = borderoptsf(sg[0:4, 0:3], [4, 3], nfase)[0:3, 0:2]
            ...
            bord[0:3, y2 - 1 : y2 + 2] = borderoptsf(
                sg[0:4, y2 - 2 : y2 + 3], [4, 5], nfase
            )[0:3, 1:-1]
    elif x2 == X - 2:
        if y2 == 0:
            bord[-3:, 0:2] = borderoptsf(sg[-4:, 0:3], [4, 3], nfase)[1:, 0:2]
            ...
            bord[-3:, y2 - 1 : y2 + 2] = borderoptsf(
                sg[-4:, y2 - 2 : y2 + 3], [4, 5], nfase
            )[1:, 1:-1]
    elif x2 == X - 1:
        if y2 == 0:
            bord[-2:, 0:2] = borderoptsf(sg[-3:, 0:3], [3, 3], nfase)[1:, 0:2]
            ...
            bord[-2:, y2 - 1 : y2 + 2] = borderoptsf(
                sg[-3:, y2 - 2 : y2 + 3], [3, 5], nfase
            )[1:, 1:-1]
    else:
        if y2 == 0:
            bord[x2 - 1 : x2 + 2, 0:2] = borderoptsf(
                sg[x2 - 2 : x2 + 3, 0:3], [5, 3], nfase
            )[1:-1, 0:2]
        elif y2 == 1:
            bord[x2 - 1 : x2 + 2, 0:3] = borderoptsf(
                sg[x2 - 2 : x2 + 3, 0:4], [5, 4], nfase
            )[1:-1, 0:3]
        elif y2 == Y - 2:
            bord[x2 - 1 : x2 + 2, -3:] = borderoptsf(
                sg[x2 - 2 : x2 + 3, -4:], [5, 4], nfase
            )[1:-1, 1:]
        elif y2 == Y - 1:
            bord[x2 - 1 : x2 + 2, -2:] = borderoptsf(
                sg[x2 - 2 : x2 + 3, -3:], [5, 3], nfase
            )[1:-1, 1:]
        else:
            bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2] = borderoptsf(
                sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3], [5, 5], nfase
            )[1:-1, 1:-1]
    return bord
...
def linear_fct(
    ...
    the rest  of the input are unnecessary informations which would only speed up your calculations for simmulated anneling, if you calculate the descriptor along its diagonal
        ...
        if boundary == "c":
            ...
            if length[0] == 0:
                ...
                for i in range(1, len(length)):
                    l_ine[i] = np.count_nonzero(
                        t[length[i] :] - t[: -length[i]] == length[i]
                    ) / (x - length[i])
                    if l_ine[i] == 0:
                        break
            else:
...
def ptdir(sg, x1, x2, y1, y2, length, lagval=999):
    ...
    if k1 == k2:
        ...
        if type(length) == int:
            strold = np.concatenate(
                (strold1, lagval * np.ones(length, dtype=int), strold2)
            )
            p = l1 + length
        else:
            strold = np.concatenate(
                (strold1, lagval * np.ones(length[-1], dtype=int), strold2)
            )
            p = l1 + length[-1]
        strnew = np.copy(strold)
...
def linput(ti, target=[0, 1], length=100, nrdir=2, sgd=[], sg=[], npoins=[]):
    if nrdir == 2:
        lf_tih0 = linear_fct(
            ti, target[0], length, orientation="h", boundary="u", row=0, col=0
        )
        lf_tih1 = linear_fct(
            ti, target[1], length, orientation="h", boundary="u", row=0, col=0
        )
        lf_tiv0 = linear_fct(
            ti, target[0], length, orientation="v", boundary="u", row=0, col=0
        )
        lf_tiv1 = linear_fct(
            ti, target[1], length, orientation="v", boundary="u", row=0, col=0
        )
        lf_oldh0 = linear_fct(
            sg, target[0], length, orientation="h", boundary="u", row=0, col=0
        )
        lf_oldh1 = linear_fct(
            sg, target[1], length, orientation="h", boundary="u", row=0, col=0
        )
        lf_oldv0 = linear_fct(
            sg, target[0], length, orientation="v", boundary="u", row=0, col=0
        )
        lf_oldv1 = linear_fct(
            sg,
            target[1],
        ...
        dold_lf = np.sum(
            (lf_tih0 / lf_tih0[0] - lf_oldh0 / lf_oldh0[0]) ** 2
            + (lf_tih1 / lf_tih1[0] - lf_oldh1 / lf_oldh1[0]) ** 2
            + (lf_tiv0 / lf_tiv0[0] - lf_oldv0 / lf_oldv0[0]) ** 2
            + (lf_tiv1 / lf_tiv1[0] - lf_oldv1 / lf_oldv1[0]) ** 2
        )
        return (
            lf_tih0,
            lf_tiv0,
        ...
        lf_tid0 = linear_fct(
            ti, target[0], int(length / 1.414), orientation="d", boundary="u"
        )
        lf_tid1 = linear_fct(
            ti, target[1], int(length / 1.414), orientation="d", boundary="u"
        )
        lf_oldh0 = linear_fct(
            sg, target[0], length, orientation="h", boundary="u", row=0, col=0
        )
        ...
        lf_oldv1 = linear_fct(
            sg, target[1], length, orientation="v", boundary="u", row=0, col=0
        )
        lf_oldd0 = linear_fct(
            sg,
            target[0],
        ...
        dold_lf = np.sum(
            ...
            (lf_tid0 / lf_tid0[0] - lf_oldd0 / lf_oldd0[0]) ** 2
            + (lf_tid1 / lf_tid1[0] - lf_oldd1 / lf_oldd1[0]) ** 2
        )
        return (
            lf_tih0,
            lf_tiv0,
        ...
        lf_tid0_1 = linear_fct(
            np.flipud(ti), target[0], int(length / 1.414), orientation="d", boundary="u"
        )
        lf_tid1_1 = linear_fct(
            np.flipud(ti), target[1], int(length / 1.414), orientation="d", boundary="u"
        )
        lf_oldh0 = linear_fct(
            sg, target[0], length, orientation="h", boundary="u", row=0, col=0
        )
        ...
        dold_lf = np.sum(
            ...
            (lf_tid0 / lf_tid0[0] - lf_oldd0 / lf_oldd0[0]) ** 2
            + (lf_tid1 / lf_tid1[0] - lf_oldd1 / lf_oldd1[0]) ** 2
            + (lf_tid0_1 / lf_tid0_1[0] - lf_oldd0_1 / lf_oldd0_1[0]) ** 2
            + (lf_tid1_1 / lf_tid1_1[0] - lf_oldd1_1 / lf_oldd1_1[0]) ** 2
        )
        return (
            lf_tih0,
            lf_tiv0,
...
def linear_fct1(
    ...
    if np.size(length) == 1:
        if boundary == "c":
            ...
            for i in range(1, length):
                l_ine[i] += (
                    np.count_nonzero(s[i:] - s[:-i] == i)
                    - np.count_nonzero(t[i:] - t[:-i] == i)
                ) / (x - i)
            ...
            if di == 1:
                ...
                if orientation == "h":
                    ...
                    if len(npoins) == 0:
                        npoins = nboun(sgd, length)
                    for i in range(1, length):
                        l_ine[i] += -np.count_nonzero(t[i:] - t[:-i] == i) / (
                            npoins[i]
                        ) + np.count_nonzero(s[i:] - s[:-i] == i) / (npoins[i])
                        if l_ine[i] == 0:
                            break
            else:
                ...
                if orientation == "h":
                    ...
                    for i in range(1, length):
                        ...
                        l_ine[i] += -np.count_nonzero(t[i:] - t[:-i] == i) / (
                            x - i * sgd[0]
                        ) + np.count_nonzero(s[i:] - s[:-i] == i) / (x - i * sgd[0])
                        if l_ine[i] == 0:
                            break
                elif orientation == "v":
                    J = (length, len(mto[0]))
                    ...
                    for i in range(1, length):
                        l_ine[i] += -np.count_nonzero(t[i:] - t[:-i] == i) / (
                            x - i * sgd[1]
                        ) + np.count_nonzero(s[i:] - s[:-i] == i) / (x - i * sgd[1])
                        if l_ine[i] == 0:
                            break
                elif orientation == "d":
                    if len(npoins) == 0:
...
def annstep_LF(
    ...
    if nrdir == 2:
        if x1 == x2:
            ...
            lf_newh0 = linear_fct1(
                sg_oldh, sg_newh, lf_oldh0, x, sgd, target[0], length, "h", "u"
            )
            lf_newh1 = linear_fct1(
                sg_oldh, sg_newh, lf_oldh1, x, sgd, target[1], length, "h", "u"
            )
            sg_newv = np.copy(sg[:, [y1, y2]])
            sg_newv[x1, 0], sg_newv[x1, 1] = sg_newv[x1, 1], sg_newv[x1, 0]
            sg_oldv = sg[:, [y1, y2]]
            lf_newv0 = linear_fct1(
                sg_oldv, sg_newv, lf_oldv0, x, sgd, target[0], length, "v", "u"
            )
            lf_newv1 = linear_fct1(
                sg_oldv, sg_newv, lf_oldv1, x, sgd, target[1], length, "v", "u"
            )
            dif_lf = np.sum(
                (lf_newh0 / lf_newh0[0] - lf_tih0 / lf_tih0[0]) ** 2
                + (lf_newv0 / lf_newv0[0] - lf_tiv0 / lf_tiv0[0]) ** 2
                + (lf_newh1 / lf_newh1[0] - lf_tih1 / lf_tih1[0]) ** 2
                + (lf_newv1 / lf_newv1[0] - lf_tiv1 / lf_tiv1[0]) ** 2
            )
        elif y1 == y2:
        ...
        if x1 == x2:
            ...
            dif_lf = np.sum(
                ...
                (lf_newd0 / lf_newd0[0] - lf_tid0 / lf_tid0[0]) ** 2
                + (lf_newd1 / lf_newd1[0] - lf_tid1 / lf_tid1[0]) ** 2
            )
        elif y1 == y2:
            sg_newh = np.copy(sg[[x1, x2], :])
            sg_newh[0, y1], sg_newh[1, y2] = sg_newh[1, y2], sg_newh[0, y1]
        ...
        if x1 == x2:
            ...
            dif_lf = np.sum(
                ...
                (lf_newd0 / lf_newd0[0] - lf_tid0 / lf_tid0[0]) ** 2
                + (lf_newd1 / lf_newd1[0] - lf_tid1 / lf_tid1[0]) ** 2
                + (lf_newd0_1 / lf_newd0_1[0] - lf_tid0_1 / lf_tid0_1[0]) ** 2
                + (lf_newd1_1 / lf_newd1_1[0] - lf_tid1_1 / lf_tid1_1[0]) ** 2
            )
        elif y1 == y2:
    ...
    if np.count_nonzero(num_points_tochange) != 0:
        ...
        for i in range(len(uniqv)):
            if num_points_tochange[i] > 0:
                ...
                pts_to_change = np.setdiff1d(
                    pts_to_change, pt_change_phase, assume_unique=True
                )
...
def gridcoarseningha(imgcoar, imagefine_old, order):
    ...
    countoccurence_up[-1, countoccurence_tobe_up] = (
        countoccurence_up[-1, countoccurence_tobe_up]
        + countoccurence[-2, countoccurence_tobe_up]
    )
    ...
    for i in range(len(order) - 1):
        ...
        if counmom[i] < coun[i]:
            ...
            for k in range(i + 1, len(order)):
                countoccurence_up[k, countoccurence_tobe_up] = (
                    countoccurence_up[k, countoccurence_tobe_up]
                    + countoccurence[i, countoccurence_tobe_up]
                )
...
def Sdet_s_l_mf(
    ...
    if nrdir == 2:
        (
            ...
            sf_oldv1,
            dold_sf,
        ) = Smfinput(ti, target, length, nrdir, sgd, sg)
        (
            lf_tih0,
            lf_tiv0,
            lf_oldh0,
            ...
            lf_oldv1,
            dold_lf,
        ) = linput(ti, target, length, nrdir, sgd, sg)
        d_old = dold_sf + dold_lf
        dini = d_old

        h = 0
        for i in range(nt):
            x1, x2, y1, y2 = choospointsf(
                bord, sgd, sg, R1[i], C1[i], O1[i], R2[i], C2[i], O2[i], nfase
            )
            dif_sf0, sf_newh0, sf_newv0 = annstep_SF(
                sg,
                sf_oldh0,
                sf_oldv0,
                sftih0,
            ...
            dif_sf1, sf_newh1, sf_newv1 = annstep_SF(
                sg,
                sf_oldh1,
                sf_oldv1,
                sftih1,
            ...
            dif_lf, lf_newh0, lf_newv0, lf_newh1, lf_newv1 = annstep_LF(
                sg,
                lf_oldh0,
                lf_oldh1,
                lf_oldv0,
            ...
            err = dif_new - d_old
            if err <= 0:
                sf_oldh0, sf_oldh1, sf_oldv0, sf_oldv1 = Smfup(
                    nrdir, sf_newh0, sf_newh1, sf_oldv0, sf_newv0, sf_newv1
                )
                lf_oldh0, lf_oldh1, lf_oldv0, lf_oldv1 = lup(
                    nrdir, lf_newh0, lf_newh1, lf_oldv0, lf_newv0, lf_newv1
                )
        ...
        for i in range(nt):
            ...
            dif_sf0, sf_newh0, sf_newv0, sf_newd0 = annstep_SF(
                sg,
                sf_oldh0,
                sf_oldv0,
                sftih0,
            ...
            dif_sf1, sf_newh1, sf_newv1, sf_newd1 = annstep_SF(
                sg,
                sf_oldh1,
                sf_oldv1,
                sftih1,
            ...
            err = dif_new - d_old
            if err <= 0:
                sf_oldh0, sf_oldh1, sf_oldv0, sf_oldv1, sf_oldd0, sf_oldd1 = Smfup(
                    nrdir,
                    sf_newh0,
                    sf_newh1,
                    sf_oldv0,
                ...
                lf_oldh0, lf_oldh1, lf_oldv0, lf_oldv1, lf_oldd0, lf_oldd1 = lup(
                    nrdir,
                    lf_newh0,
                    lf_newh1,
                    lf_oldv0,
        ...
        (
            ...
            sf_oldd0_1,
            sf_oldd1_1,
        ) = Smfinput(ti, target, length, nrdir, sgd, sg, npoins)
        (
            lf_tih0,
            lf_tiv0,
            lf_oldh0,
            ...
            lf_oldd0_1,
            lf_oldd1_1,
        ) = linput(ti, target, length, nrdir, sgd, sg, npoins)
        d_old = dold_sf + dold_lf
        dini = d_old
        ...
        for i in range(nt):
            ...
            (
                ...
                sg_d1new,
                sg_d1old,
            ) = annstep(sg, x1, x2, y1, y2, sgd, nrdir, length, nfase)
...
def siman_s_l_mf(
    ...
    if nrdir == 2:
        (
            ...
            C2,
            O2,
        ) = Sdet_s_l_mf(ti, target, length, nps, nt, nfase, sgd, nrdir, sg)
    elif nrdir == 3:
        (
            sg,
            sgini,
    ...
    if nrdir == 2:
        for J in range(nt):
            x1, x2, y1, y2 = choospointsf(
                bord, sgd, sg, R1[J], C1[J], O1[J], R2[J], C2[J], O2[J], nfase
            )
            dif_sf0, sf_newh0, sf_newv0 = annstep_SF(
                sg,
                sf_oldh0,
            ...
            err = dif_new - d_old
            # if J%5000==0:
            # rui.inform(J,d_old, dif_sf,dif_lf)
            if err <= 0:
                sf_oldh0, sf_oldh1, sf_oldv0, sf_oldv1 = Smfup(
                    nrdir, sf_newh0, sf_newh1, sf_oldv0, sf_newv0, sf_newv1
                )
        ...
        for J in range(nt):
            ...
            err = dif_new - d_old
            # if J%5000==0:
            # rui.inform("Iteration",J,"E",d_old)
            # rui.inform("N", n,"S2", dif_sf,"L2",dif_lf)
            if err <= 0:
                (
                    sf_oldh0,
                    sf_oldh1,
...
def siman_s_l_grf1(
    ...
    if gridlevel == 5:
        ...
        sg0 = siman_s_l_mf(
            ti0, sgini0, target, length0, nps, nt, nstop, acc, nfase, sgd0, lam, nrdir
        )[0]
        ...
        sg0 = siman_s_l_mf(
            ti0, sg, target, length0, nps, nt, nstop, acc, nfase, sgd0, lam, nrdir
        )[0]
        return sg0


def siman_s_l_grf_mf(
    ti0,
    sgini,
    target=[0, 1],
    length0=100,
    ...
    if gridlevel == 5:
        ...
        sg0 = siman_s_l_mf(
            ti0, sgini, target, length0, nps, nt, nstop, acc, nfase, sgd0, lam, nrdir
        )[0]
        return sg0
...
def Smfinput(ti, target=[0, 1], length=100, nrdir=2, sgd=[], sg=[], npoins=[]):
    ...
    if nrdir == 2:
        ...
        dold_sf = np.sum(
            (sftih0 / sftih0[0] - sf_oldh0 / sf_oldh0[0]) ** 2
            + (sftiv0 / sftiv0[0] - sf_oldv0 / sf_oldv0[0]) ** 2
            + (sftih1 / sftih1[0] - sf_oldh1 / sf_oldh1[0]) ** 2
            + (sftiv1 / sftiv1[0] - sf_oldv1 / sf_oldv1[0]) ** 2
        )
        return (
            sftih0,
            sftiv0,
        ...
        sf_oldv1 = same_facies(sg, tar1, length, orientation="v", boundary="u")
        sftid0 = same_facies(
            ti, tar0, int(length / 1.414), orientation="d", boundary="u"
        )
        sftid1 = same_facies(
            ti, tar1, int(length / 1.414), orientation="d", boundary="u"
        )
        sf_oldd0 = same_facies(
            sg,
            tar0,
            int(length / 1.414),
            orientation="d",
        ...
        dold_sf = np.sum(
            ...
            + (sftih1 / sftih1[0] - sf_oldh1 / sf_oldh1[0]) ** 2
            + (sftiv1 / sftiv1[0] - sf_oldv1 / sf_oldv1[0]) ** 2
            + (sftid0 / sftid0[0] - sf_oldd0 / sf_oldd0[0]) ** 2
            + (sftid1 / sftid1[0] - sf_oldd1 / sf_oldd1[0]) ** 2
        )
        return (
            sftih0,
            sftiv0,
        ...
        sftid0_1 = same_facies(
            np.flipud(ti), tar0, int(length / 1.414), orientation="d", boundary="u"
        )
        sftid1_1 = same_facies(
            np.flipud(ti), tar1, int(length / 1.414), orientation="d", boundary="u"
        )
        sf_oldd0_1 = same_facies(
            np.flipud(sg),
            tar0,
            int(length / 1.414),
            orientation="d",
        ...
        dold_sf = np.sum(
            ...
            (sftid0 / sftid0[0] - sf_oldd0 / sf_oldd0[0]) ** 2
            + (sftid1 / sftid1[0] - sf_oldd1 / sf_oldd1[0]) ** 2
            + (sftid0_1 / sftid0_1[0] - sf_oldd0_1 / sf_oldd0_1[0]) ** 2
            + (sftid1_1 / sftid1_1[0] - sf_oldd1_1 / sf_oldd1_1[0]) ** 2
        )
        return (
            sftih0,
            sftiv0,
            ...
            if orientation == "h":
                ...
                if np.size(target) == 1:
                    ...
                    for i in range(1, length):
                        ...
                        s_f[i] = np.count_nonzero(mt[:-i] + mt[i:] == 2 * target) / (
                            npoins[i]
                        )
        ...
        if boundary == "c":
            ...
            if np.size(target) == 1:
                ...
                if length[0] == 0:
                    ...
                    for i in range(1, len(length)):
                        s_f[i] = np.count_nonzero(
                            mt[: -length[i]] + mt[length[i] :] == 2 * target
                        ) / (x - length[i])
                else:
                    for i in range(len(length)):
                        s_f[i] = np.count_nonzero(
                ...
                if length[0] == 0:
                    ...
                    for i in range(1, len(length)):
                        ...
                        for J in range(0, len(target)):
                            ...
                            s_f[i, J] = np.count_nonzero(k == target[J]) / (
                                x - length[i]
                            )
                else:
                    for i in range(len(length)):
                    ...
                    if length[0] == 0:
                        ...
                        for i in range(1, len(length)):
                            ...
                            for l in range(0, len(target)):
                                ...
                                s_f[i, l] = np.count_nonzero(k == target[l]) / (
                                    x - length[i] * s
                                )
                    else:
                        for i in range(len(length)):
    ...
    if np.size(length) == 1:
        if boundary == "c":
            ...
            if np.size(target) == 1:
                s_f = np.copy(sfold)
                for i in range(1, length):
                    s_f[i] += (np.count_nonzero(mtn[:-i] + mtn[i:] == 2 * target)) / (
                        x - i
                    ) - np.count_nonzero(mto[:-i] + mto[i:] == 2 * target) / (x - i)
            else:
                s_f = np.copy(sfold)
                for i in range(1, length):
                    k = mto[:-i] + mto[i:]
                    ...
                    for J in range(0, len(target)):
                        ...
                        s_f[i, J] += np.count_nonzero(k == target[J]) / (
                            x - i
                        ) - np.count_nonzero(k == target[J]) / (x - i)
            ...
            if orientation == "h":
                if mto.ndim == 1:
                    if np.size(target) == 1:
                        ...
                        for i in range(1, length):
                            s_f[i] += (
                                np.count_nonzero(mtn[:-i] + mtn[i:] == 2 * target)
                                - np.count_nonzero(mto[:-i] + mto[i:] == 2 * target)
                            ) / (x - i * sgd[0])
                    else:
                        s_f = np.copy(sfold)
                        for i in range(1, length):
                            ...
                            l = mtn[:-i] + mtn[i:]
                            for J in range(0, len(target)):
                                s_f[i, J] += np.count_nonzero(l == target[J]) / (
                                    x - i
                                ) - np.count_nonzero(k == target[J]) / (x - i * sgd[0])
                else:
                    s = sgd[0]
                    if np.size(target) == 1:
                        s_f = np.copy(sfold)
                        ...
                        for i in range(1, length):
                            ...
                            for J in range(0, len(target)):
                                s_f[i, J] += np.count_nonzero(l == target[J]) / (
                                    x - i * s
                                ) - np.count_nonzero(k == target[J]) / (x - i * s)
            elif orientation == "v":
                if mto.ndim == 1:
                    ...
                    if np.size(target) == 1:
                        ...
                        for i in range(1, length):
                            s_f[i] += np.count_nonzero(
                                mtn[:-i] + mtn[i:] == 2 * target
                            ) / (x - i * sgd[1]) - np.count_nonzero(
                                mto[:-i] + mto[i:] == 2 * target
                            ) / (
                                x - i * sgd[1]
                            )
                        ...
                        for i in range(1, length):
                            ...
                            for J in range(0, len(target)):
                                s_f[i, J] += np.count_nonzero(l == target[J]) / (
                                    x - i * sgd[1]
                                ) - np.count_nonzero(k == target[J]) / (x - i * sgd[1])
                else:
                    s = sgd[1]
                    if np.size(target) == 1:
                        s_f = np.copy(sfold)
                ...
                if np.size(target) == 1:
                    ...
                    for i in range(1, length):
                        ...
                        for J in range(0, len(target)):
                            s_f[i, J] += np.count_nonzero(l == target[J]) / (
                                npoins[i]
                            ) - np.count_nonzero(k == target[J]) / (npoins[i])
...
def annstep_SF(
    ...
    if nrdir == 2:
        if x1 == x2:
            ...
            sg_newh[y1], sg_newh[y2] = sg_newh[y2], sg_newh[y1]
            sg_oldh = np.copy(sg[x1, :])
            sf_newh = same_facies1(
                sg_oldh, sg_newh, sf_oldh, x, sgd, target, length, "h", "u"
            )
            sg_newv = np.copy(sg[:, [y1, y2]])
            sg_newv[x1, 0], sg_newv[x1, 1] = sg_newv[x1, 1], sg_newv[x1, 0]
            sg_oldv = np.copy(sg[:, [y1, y2]])
            sf_newv = same_facies1(
                sg_oldv, sg_newv, sf_oldv, x, sgd, target, length, "v", "u"
            )
            dif_sf = np.sum(
                (sf_newh / sf_newh[0] - sftih / sftih[0]) ** 2
                + (sf_newv / sf_newv[0] - sftiv / sftiv[0]) ** 2
            )
        elif y1 == y2:
            sg_newh = np.copy(sg[[x1, x2], :])
            sg_newh[0, y1], sg_newh[1, y2] = sg_newh[1, y2], sg_newh[0, y1]
        ...
        if x1 == x2:
            ...
            dif_sf = np.sum(
                (sf_newh / sf_newh[0] - sftih / sftih[0]) ** 2
                + (sf_newv / sf_newv[0] - sftiv / sftiv[0]) ** 2
            ) + np.sum((sf_newd / sf_newd[0] - sftid / sftid[0]) ** 2)
        ...
        if x1 == x2:
            ...
            dif_sf = np.sum(
                ...
                (sf_newd / sf_newd[0] - sftid / sftid[0]) ** 2
                + (sf_newd_1 / sf_newd_1[0] - sftid_1 / sftid_1[0]) ** 2
            )
...
def Mphistini7mf(sg, arr):
    ...
    M = np.size(sg)
    M = float(M)
    mphist = ndi.measurements.labeled_comprehension(
        mt_2, mt_2, arr, np.count_nonzero, int, 0
    )
...
def Mphistup7mf(sg, arr, x, x1, x2, y1, y2, mphistold, sgd):
    ...
    mphist1o = ndi.measurements.labeled_comprehension(
        Mt_1o, Mt_1o, arr, np.count_nonzero, int, 0
    )
    mphist1o[0] = np.count_nonzero(Mt_1o == arr[0])
    mphist1n = ndi.measurements.labeled_comprehension(
        Mt_1n, Mt_1n, arr, np.count_nonzero, int, 0
    )
    mphist1n[0] = np.count_nonzero(Mt_1n == arr[0])
    mphist2o = ndi.measurements.labeled_comprehension(
        Mt_2o, Mt_2o, arr, np.count_nonzero, int, 0
    )
    mphist2o[0] = np.count_nonzero(Mt_2o == arr[0])
    mphist2n = ndi.measurements.labeled_comprehension(
        Mt_2n, Mt_2n, arr, np.count_nonzero, int, 0
    )
...
def poresize(mt1, target=1, bins=np.arange(0.1, 1, 0.1), neighbors=4, lab=[], k=0):
    ...
    if len(lab) == 0 or (k) == 0:
        lab, k = label_fct(
            mt1, target, neighbors, 1
        )  # Calculates the labeld components if not provided beforehand
    binc = np.bincount(np.reshape(lab, -1))[
        1:
    ]  # counting the number of particles in each connected componenent
    ...
    for I in range(len(pore_mean)):
        pore_mean[I] = np.mean(
            binc_sort10[bins1[I] : bins1[I + 1]]
        )  # calculates the mean value of each class
    return pore_mean, binc


def poreper(
    mt1, target=0, bins=np.arange(0.1, 1, 0.10), Neighbors=4, lab=[], k=0, bord=[]
):
    if len(lab) == 0 or k == 0:
...
def c1input(ti, target=0, length=100, nrdir=2, sgd=[], sg=[], npoins=[], Neighbors=4):
    ...
    if nrdir == 2:
        ...
        cf_tih0 = cluster_fct(
            ti, target, length, "h", "u", row, col, sgd, npoins, Neighbors
        )
        cf_tiv0 = cluster_fct(
            ti, target, length, "v", "u", row, col, sgd, npoins, Neighbors
        )
        cf_oldh0 = cluster_fct(
            sg, target, length, "h", "u", row, col, sgd, npoins, Neighbors, lold0, posin
        )
        cf_oldv0 = cluster_fct(
            sg, target, length, "v", "u", row, col, sgd, npoins, Neighbors, lold0, posin
        )
        dclu_old0 = np.sum(
            (cf_tih0 / cf_tih0[0] - cf_oldh0 / cf_oldh0[0]) ** 2
            + (cf_tiv0 / cf_tiv0[0] - cf_oldv0 / cf_oldv0[0]) ** 2
        )
        ...
        dclu_old0 = np.sum(
            (cf_tih0 / cf_tih0[0] - cf_oldh0 / cf_oldh0[0]) ** 2
            + (cf_tiv0 / cf_tiv0[0] - cf_oldv0 / cf_oldv0[0]) ** 2
        ) + np.sum((cf_tid0 / cf_tid0[0] - cf_oldd0 / cf_oldd0[0]) ** 2)
        ...
        cf_tiv0 = cluster_fct(ti, target, length, "v", "u")
        cf_tid0 = cluster_fct(ti, target, int(length / 1.414), "d", "u")
        cf_tid0_1 = cluster_fct(
            np.flipud(np.copy(ti)), target, int(length / 1.414), "d", "u"
        )
        cf_oldh0 = cluster_fct(
            sg, target, length, "h", "u", row, col, sgd, npoins, Neighbors, lold0, posin
        )
        ...
        cf_oldd0_1 = cluster_fct(
            np.flipud(np.copy(sg)),
            target,
            int(length / 1.414),
            "d",
            "u",
        ...
        dclu_old0 = np.sum(
            ...
            (cf_tid0 / cf_tid0[0] - cf_oldd0 / cf_oldd0[0]) ** 2
            + (cf_tid0_1 / cf_tid0_1[0] - cf_oldd0_1 / cf_oldd0_1[0]) ** 2
        )
        return (
            lold0,
            cf_tih0,
    ...
    if np.size(length) == 1:
        if boundary == "c":
            ...
            if orientation == "h":
                ...
                mt0 = mt != 0
                for i in range(1, length):
                    c_f[i] = np.count_nonzero((mt[:-i] == mt[i:]) * mt0[:-i]) / (
                        x - i * j[0]
                    )
            elif orientation == "v":
                j = (length, len(mt[0]))
        ...
        if boundary == "c":
            ...
            if length[0] == 0:
                c_f[0] = np.count_nonzero(mt != 0) / x
                for i in range(1, len(length)):
                    c_f[i] = np.count_nonzero(
                        (mt[: -length[i]] == mt[length[i] :]) * mt0[: -length[i]]
                    ) / (x - length[i])
            else:
                for i in range(len(length)):
                    c_f[i] = np.count_nonzero(
...
def annstep_CF1f(
    ...
    if nrdir == 2:
        if up0 == 0:
            cf_newh0 = cluster_fct1(
                lold0, lnew0, cf_oldh0, x, x1, x2, y1, y2, sgd, length, "h"
            )
            cf_newv0 = cluster_fct1(
                lold0, lnew0, cf_oldv0, x, x1, x2, y1, y2, sgd, length, "v"
            )
        elif up0 == 1:
            cf_newh0 = cluster_fct(
                [], [], length, "h", "u", row=0, col=0, Neighbors=4, label=lnew0
            )
            cf_newv0 = cluster_fct(
                [], [], length, "v", "u", row=0, col=0, Neighbors=4, label=lnew0
            )

        dif_cf0 = np.sum(
            (cf_newh0[1:] / cf_newh0[0] - cf_tih0[1:] / cf_tih0[0]) ** 2
            + (cf_newv0[1:] / cf_newv0[0] - cf_tiv0[1:] / cf_tiv0[0]) ** 2
        )
        return (
            dif_cf0,
            cf_newh0,
        ...
        dif_cf0 = np.sum(
            (cf_newh0[1:] / cf_newh0[0] - cf_tih0[1:] / cf_tih0[0]) ** 2
            + (cf_newv0[1:] / cf_newv0[0] - cf_tiv0[1:] / cf_tiv0[0]) ** 2
        ) + np.sum((cf_newd0 / cf_newd0[0] - cf_tid0 / cf_tid0[0]) ** 2)
        ...
        if up0 == 0:
            ...
            cf_newd0_1 = cluster_fct(
                ...
                col,
                Neighbors=4,
                label=np.flipud(lnew0),
                npoins=npoins,
                posin=posin,
            )

        dif_cf0 = np.sum(
            (cf_tih0 / cf_tih0[0] - cf_newh0 / cf_newh0[0]) ** 2
            + (cf_tiv0 / cf_tiv0[0] - cf_newv0 / cf_newv0[0]) ** 2
        ) + np.sum(
            (cf_tid0 / cf_tid0[0] - cf_newd0 / cf_newd0[0]) ** 2
            + (cf_tid0_1 / cf_tid0_1[0] - cf_newd0_1 / cf_newd0_1[0]) ** 2
        )
    ...
    if np.size(length) == 1:
        ...
        if orientation == "h":
            if x1 == x2:
                ...
                for i in range(1, length):
                    c_f[i] += np.count_nonzero(
                        (mtln[:-i] == mtln[i:]) * mtlnbin[:-i]
                    ) / (x - sgd[0] * i) - np.count_nonzero(
                        (mtlo[:-i] == mtlo[i:]) * mtlobin[:-i]
                    ) / (
                        x - sgd[0] * i
                    )
            ...
            for i in range(1, length):
                c_f[i] += (
                    np.count_nonzero((mtln1[:-i] == mtln1[i:]) * mtlnbin[:-i])
                    - np.count_nonzero((mtlo1[:-i] == mtlo1[i:]) * mtlobin[:-i])
                ) / (npoins[i])
...
def errorf(ti1, sg1, target, length, Neighbors=4, bins=np.arange(0.1, 1, 0.1), nrdir=4):
    ...
    sftid_1 = same_facies(
        np.flipud(ti), target, int(length / 1.414), orientation="d", boundary="u"
    )
    sf_oldd = same_facies(
        sg,
        target,
    ...
    d_sf = np.sum(
        (sftih / sftih[0] - sf_oldh / sf_oldh[0]) ** 2
        + (sftiv / sftiv[0] - sf_oldv / sf_oldv[0]) ** 2
    ) + np.sum(
        (sftid / sftid[0] - sf_oldd / sf_oldd[0]) ** 2
        + (sftid_1 / sftid_1[0] - sf_oldd_1 / sf_oldd_1[0]) ** 2
    )
    lf_tih0 = linear_fct(
        ti, target, length, orientation="h", boundary="u", row=0, col=0
    )
    lf_tiv0 = linear_fct(
        ti, target, length, orientation="v", boundary="u", row=0, col=0
    )
    ...
    lf_oldh0 = linear_fct(
        sg, target, length, orientation="h", boundary="u", row=0, col=0
    )
    lf_oldv0 = linear_fct(
        sg, target, length, orientation="v", boundary="u", row=0, col=0
    )
    lf_oldd0 = linear_fct(
        sg,
        target,
    ...
    (
        ...
        cf_tid0_1,
        cf_oldd0_1,
    ) = c1input(ti, target, length, nrdir, sgd, sg, npoins)
    ...
    if nrdir == 2:
        if y1 == y2:
                ...
                if dif <= 3 * length:
                    if x1 < length:
                        ...
                        if x2 > sgd[0] - length:
                            ...
                            sgoldv = np.transpose(sgoldv)
                            sgnewv = np.copy(sgoldv)
                            sgnewv[length], sgnewv[length + dif] = (
                                sgnewv[length + dif],
                                sgnewv[length + dif],
                            )
                        else:
                    ...
                    sgoldv = np.append(
                        sgoldv0,
                        np.append(999 * np.ones(int(length), dtype=int), sgoldv1),
                    )
                    sgnewv = np.append(
                        sgnewv0,
                        np.append(999 * np.ones(int(length), dtype=int), sgnewv1),
                    )
            else:
                dif = x1 - x2
                if dif <= 3 * length:
            ...
            if x2 < length:
                ...
                sgnewv1[length] = nfase[0]
            sgoldv = np.append(
                sgoldv0, np.append(999 * np.ones(int(length), dtype=int), sgoldv1)
            )
            sgnewv = np.append(
                sgnewv0, np.append(999 * np.ones(int(length), dtype=int), sgnewv1)
            )
        ...
        if x1 == x2:
                ...
                if dif <= 3 * length:
                    if y1 < length:
                        ...
                        if y2 > sgd[1] - length:
                            ...
                            sgnewh = np.copy(sgoldh)
                            sgnewh[length], sgnewh[length + dif] = (
                                sgnewh[length + dif],
                                sgnewh[length],
                            )
                        else:
                    ...
                    sgoldh = np.append(
                        sgoldh0,
                        np.append(999 * np.ones(int(length), dtype=int), sgoldh1),
                    )
                    sgnewh = np.append(
                        sgnewh0,
                        np.append(999 * np.ones(int(length), dtype=int), sgnewh1),
                    )
            else:
                dif = y1 - y2
                if dif <= 3 * length:
            ...
            if y2 < length:
                ...
                sgnewh1[length] = nfase[0]
            sgoldh = np.append(
                sgoldh0, np.append(999 * np.ones(int(length), dtype=int), sgoldh1)
            )
            sgnewh = np.append(
                sgnewh0, np.append(999 * np.ones(int(length), dtype=int), sgnewh1)
            )
        return sgoldh, sgnewh, sgoldv, sgnewv
    elif nrdir == 3:
        sgoldh, sgnewh, sgoldv, sgnewv = annstep(
            sg, x1, x2, y1, y2, sgd, 2, length, nfase
        )
        k1 = y1 - x1
        k2 = y2 - x2
        if k1 == k2:
            ...
            sg_d0new = np.append(
                sg_newxy0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newxy1),
            )
            sg_d0old = np.append(
                sg_oldxy0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldxy1),
            )
        return sgoldh, sgnewh, sgoldv, sgnewv, sg_d0new, sg_d0old
    elif nrdir == 4:
        sgoldh, sgnewh, sgoldv, sgnewv, sg_d0new, sg_d0old = annstep(
            sg, x1, x2, y1, y2, sgd, 3, length, nfase
        )
        sgapp = np.flipud(sg)
        x1app = sgd[0] - 1 - x1
        x2app = sgd[0] - 1 - x2
        ...
        if k1 == k2:
            ...
            sg_newxy0, sg_oldxy0 = ptdirup_2d(
                sgapp, x1app, y1, nfase[1], int(length / 1.414)
            )
            sg_newxy1, sg_oldxy1 = ptdirup_2d(
                sgapp, x2app, y2, nfase[0], int(length / 1.414)
            )
            sg_d1new = np.append(
                sg_newxy0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newxy1),
...
def same_facies_up(
    mto, mtn, sfold0, sfold1, x, sizetosub, target, length, orientation="h", npoins=[]
):
    s_f0 = np.copy(sfold0)
    s_f1 = np.copy(sfold1)
    if orientation == "d":
        ...
        for i in range(1, length):
            ...
            s_f0[i] += np.count_nonzero(k == 2 * target[0]) / (
                npoins[i]
            ) - np.count_nonzero(l == 2 * target[0]) / (npoins[i])
            s_f1[i] += np.count_nonzero(k == 2 * target[1]) / (
                npoins[i]
            ) - np.count_nonzero(l == 2 * target[1]) / (npoins[i])
    else:
        for i in range(1, length):
            k = mtn[:-i] + mtn[i:]
            l = mto[:-i] + mto[i:]
            ...
            s_f0[i] += (
                np.count_nonzero(k == 2 * target[0])
                - np.count_nonzero(l == 2 * target[0])
            ) / (x - i * sizetosub)
            s_f1[i] += (
                np.count_nonzero(k == 2 * target[1])
                - np.count_nonzero(l == 2 * target[1])
            ) / (x - i * sizetosub)
    return s_f0, s_f1


def linear_fct_up(
    mto, mtn, linold, x, sizetosub, target, length, orientation="h", npoins=[]
):
    l_ine = np.copy(linold)
    t = np.nonzero(mto == target)[0]
    s = np.nonzero(mtn == target)[0]
...
def annstep_sf2d(
    ...
    if nrdir == 2:
        sf_newh0, sf_newh1 = same_facies_up(
            sg_oldh, sg_newh, sf_oldh0, sf_oldh1, x, sgd[0], target, length, "h"
        )
        sf_newv0, sf_newv1 = same_facies_up(
            sg_oldv, sg_newv, sf_oldv0, sf_oldv1, x, sgd[1], target, length, "v"
        )
        dif_sf = np.sum(
            (sftih0 / sftih0[0] - sf_newh0 / sf_newh0[0]) ** 2
            + (sftiv0 / sftiv0[0] - sf_newv0 / sf_newv0[0]) ** 2
            + (sftih1 / sftih1[0] - sf_newh1 / sf_newh1[0]) ** 2
            + (sftiv1 / sftiv1[0] - sf_newv1 / sf_newv1[0]) ** 2
        )
        ...
        sf_newd0, sf_newd1, = same_facies_up(
            sg_d0old,
            sg_d0new,
            sf_oldd0,
            sf_oldd1,
        ...
        dif_sf = np.sum(
            ...
            (sftid0 / sftid0[0] - sf_newd0 / sf_newd0[0]) ** 2
            + (sftid1 / sftid1[0] - sf_newd1 / sf_newd1[0]) ** 2
        )
        ...
        sf_newd0_1, sf_newd1_1, = same_facies_up(
            sg_d1old,
            sg_d1new,
            sf_oldd0_1,
            sf_oldd1_1,
        ...
        dif_sf = np.sum(
            ...
            (sftid0 / sftid0[0] - sf_newd0 / sf_newd0[0]) ** 2
            + (sftid1 / sftid1[0] - sf_newd1 / sf_newd1[0]) ** 2
            + (sftid0_1 / sftid0_1[0] - sf_newd0_1 / sf_newd0_1[0]) ** 2
            + (sftid1_1 / sftid1_1[0] - sf_newd1_1 / sf_newd1_1[0]) ** 2
        )
        return (
            dif_sf,
            sf_newh0,
...
def annstep_lf2d(
    ...
    if nrdir == 2:
        lf_newh0 = linear_fct_up(
            sg_oldh, sg_newh, lf_oldh0, x, sgd[0], target[0], length, "h"
        )
        lf_newh1 = linear_fct_up(
            sg_oldh, sg_newh, lf_oldh1, x, sgd[0], target[1], length, "h"
        )
        lf_newv0 = linear_fct_up(
            sg_oldv, sg_newv, lf_oldv0, x, sgd[1], target[0], length, "v"
        )
        lf_newv1 = linear_fct_up(
            sg_oldv, sg_newv, lf_oldv1, x, sgd[1], target[1], length, "v"
        )
        dif_lf = np.sum(
            (lf_newh0 / lf_newh0[0] - lf_tih0 / lf_tih0[0]) ** 2
            + (lf_newv0 / lf_newv0[0] - lf_tiv0 / lf_tiv0[0]) ** 2
...
def inputrelable(TI, order):
    ...
    if (
        np.array_equal(order, range(1, len(order + 1))) == 0
    ):  # reseting order in such a way that it goes 1,2,..
        for i in range(len(order)):
...
def Sdet_l_s_3d(
    ...
    if nrdir == 3:
        (
            ...
            sf_oldz1,
            sftiz1,
        ) = Smfinput3d(ti, target, length, nrdir, sgd, sg)
        (
            lf_tih0,
            lf_tiv0,
            lf_tiz0,
            ...
            lf_oldz1,
            dold_lf,
        ) = linput3d(ti, target, length, nrdir, sgd, sg)
        d_old = d_oldsf + dold_lf
        dini = d_old
        h = 0
        for i in range(nt):
            x1, x2, y1, y2, z1, z2 = choospointsf_3d(
                bord,
                sgd,
                sg,
                xp1[i],
            ...
            sg_oldh, sg_newh, sg_oldv, sg_newv, sg_oldz, sg_newz = annstep3d(
                sg, x1, x2, y1, y2, z1, z2, sgd, nrdir, length, nfase
            )
            (
                dif_lf,
                lf_newh0,
            ...
            err = dif_new - d_old
            if err <= 0:
                sf_oldh0, sf_oldh1, sf_oldv0, sf_oldv1, sf_oldz0, sf_oldz1 = sup3d(
                    nrdir, sf_newh0, sf_newh1, sf_newv0, sf_newv1, sf_newz0, sf_newz1
                )
                lf_oldh0, lf_oldh1, lf_oldv0, lf_oldv1, lf_oldz0, lf_oldz1 = lup3d(
                    nrdir, lf_newh0, lf_newh1, lf_newv0, lf_newv1, lf_newz0, lf_newz1
                )
                d_old = dif_new
        ...
        for i in range(nt):
            ...
            (
                ...
                sg_oldyz,
                sg_newyz,
            ) = annstep3d(sg, x1, x2, y1, y2, z1, z2, sgd, nrdir, length, target)
            (
                dif_sf,
                sf_newh0,
                sf_newv0,
...
def siman_l_s_3d(
    ...
    if nrdir == 3:
        (
            ...
            lf_oldz0,
            lf_oldz1,
        ) = Sdet_l_s_3d(ti, target, length, nps, nt, nrdir, nfase, sgd, sg)
    elif nrdir == 6:
        (
            sg,
            sgini,
        ...
        for J in range(nt):
            ...
            if err <= 0:
                ...
                if ran[J] <= K:
                    ...
                    d_old = dif_new
                    sg[x1, y1, z1], sg[x2, y2, z2] = sg[x2, y2, z2], sg[x1, y1, z1]
                    bord = borderoptadoptsf3D(
                        sg, bord, x1, x2, y1, y2, z1, z2, sgd, nfase
                    )
                    n = 0
                else:
...
def borderoptsf3D(sg, sgd, nfase):
    ...
    mt_2 = np.fabs(
        (
            6
            - (mt_1[:-2, 1:-1, 1:-1] == mt_1[1:-1, 1:-1, 1:-1])
            - (mt_1[2:, 1:-1, 1:-1] == mt_1[1:-1, 1:-1, 1:-1])
            - (mt_1[1:-1, 2:, 1:-1] == mt_1[1:-1, 1:-1, 1:-1])
            - (mt_1[1:-1, :-2, 1:-1] == mt_1[1:-1, 1:-1, 1:-1])
            - (mt_1[1:-1, 2:, 1:-1] == mt_1[1:-1, 1:-1, 2:])
            - (mt_1[1:-1, 1:-1, :-2] == mt_1[1:-1, 1:-1, 1:-1])
        )
    )
    mt_2 = mt_2.astype(int)
...
def borderoptadoptsf3D(sg, bord, x1, x2, y1, y2, z1, z2, sgd, nfase):
    ...
    if x1 == 0:
        if y1 == 0:
            if z1 == 0:
                bord[0:2, 0:2, 0:2] = borderoptsf3D(
                    (sg[0:3, 0:3, 0:3]), [3, 3, 3], nfase
                )[0:2, 0:2, 0:2]
            elif z1 == 1:
                bord[0:2, 0:2, 0:3] = borderoptsf3D(
                    (sg[0:3, 0:3, 0:4]), [3, 3, 4], nfase
                )[0:2, 0:2, 0:3]
            elif z1 == Z - 2:
                bord[0:2, 0:2, -3:] = borderoptsf3D(
                    (sg[0:3, 0:3, -4:]), [3, 3, 4], nfase
                )[0:2, 0:2, 1:]
            elif z1 == Z - 1:
                bord[0:2, 0:2, -2:] = borderoptsf3D(
                    (sg[0:3, 0:3, -3:]), [3, 3, 3], nfase
                )[0:2, 0:2, 1:]
            else:
                bord[0:2, 0:2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:3, 0:3, z1 - 2 : z1 + 3]), [3, 3, 5], nfase
                )[0:2, 0:2, 1:-1]
        elif y1 == 1:
            if z1 == 0:
                bord[0:2, 0:3, 0:2] = borderoptsf3D(
                    (sg[0:3, 0:4, 0:3]), [3, 4, 3], nfase
                )[0:2, 0:3, 0:2]
            elif z1 == 1:
                bord[0:2, 0:3, 0:3] = borderoptsf3D(
                    (sg[0:3, 0:4, 0:4]), [3, 4, 4], nfase
                )[0:2, 0:3, 0:3]
            elif z1 == Z - 2:
                bord[0:2, 0:3, -3:] = borderoptsf3D(
                    (sg[0:3, 0:4, -4:]), [3, 4, 4], nfase
                )[0:2, 0:3, 1:]
            elif z1 == Z - 1:
                bord[0:2, 0:3, -2:] = borderoptsf3D(
                    (sg[0:3, 0:4, -3:]), [3, 4, 3], nfase
                )[0:2, 0:3, 1:]
            else:
                bord[0:2, 0:3, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:3, 0:4, z1 - 2 : z1 + 3]), [3, 4, 5], nfase
                )[0:2, 0:3, 1:-1]
        elif y1 == Y - 2:
            if z1 == 0:
                bord[0:2, -3:, 0:2] = borderoptsf3D(
                    (sg[0:3, -4:, 0:3]), [3, 4, 3], nfase
                )[0:2, 1:, 0:2]
            elif z1 == 1:
                bord[0:2, -3:, 0:3] = borderoptsf3D(
                    (sg[0:3, -4:, 0:4]), [3, 4, 4], nfase
                )[0:2, 1:, 0:3]
            elif z1 == Z - 2:
                bord[0:2, -3:, -3:] = borderoptsf3D(
                    (sg[0:3, -4:, -4:]), [3, 4, 4], nfase
                )[0:2, 1:, 1:]
            elif z1 == Z - 1:
                bord[0:2, -3:, -2:] = borderoptsf3D(
                    (sg[0:3, -4:, -3:]), [3, 4, 3], nfase
                )[0:2, 1:, 1:]
            else:
                bord[0:2, -3:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:3, -4:, z1 - 2 : z1 + 3]), [3, 4, 5], nfase
                )[0:2, 1:, 1:-1]
        elif y1 == Y - 1:
            if z1 == 0:
                bord[0:2, -2:, 0:2] = borderoptsf3D(
                    (sg[0:3, -3:, 0:3]), [3, 3, 3], nfase
                )[0:2, 1:, 0:2]
            elif z1 == 1:
                bord[0:2, -2:, 0:3] = borderoptsf3D(
                    (sg[0:3, -3:, 0:4]), [3, 3, 4], nfase
                )[0:2, 1:, 0:3]
            elif z1 == Z - 2:
                bord[0:2, -2:, -3:] = borderoptsf3D(
                    (sg[0:3, -3:, -4:]), [3, 3, 4], nfase
                )[0:2, 1:, 1:]
            elif z1 == Z - 1:
                bord[0:2, -2:, -2:] = borderoptsf3D(
                    (sg[0:3, -3:, -3:]), [3, 3, 3], nfase
                )[0:2, 1:, 1:]
            else:
                bord[0:2, -2:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:3, -3:, z1 - 2 : z1 + 3]), [3, 3, 5], nfase
                )[0:2, 1:, 1:-1]
        else:
            if z1 == 0:
                bord[0:2, y1 - 1 : y1 + 2, 0:2] = borderoptsf3D(
                    (sg[0:3, y1 - 2 : y1 + 3, 0:3]), [3, 5, 3], nfase
                )[0:2, 1:-1, 0:2]
            elif z1 == 1:
                bord[0:2, y1 - 1 : y1 + 2, 0:3] = borderoptsf3D(
                    (sg[0:3, y1 - 2 : y1 + 3, 0:4]), [3, 5, 4], nfase
                )[0:2, 1:-1, 0:3]
            elif z1 == Z - 2:
                bord[0:2, y1 - 1 : y1 + 2, -3:] = borderoptsf3D(
                    (sg[0:3, y1 - 2 : y1 + 3, -4:]), [3, 5, 4], nfase
                )[0:2, 1:-1, 1:]
            elif z1 == Z - 1:
                bord[0:2, y1 - 1 : y1 + 2, -2:] = borderoptsf3D(
                    (sg[0:3, y1 - 2 : y1 + 3, -3:]), [3, 5, 3], nfase
                )[0:2, 1:-1, 1:]
            else:
                bord[0:2, y1 - 1 : y1 + 2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:3, y1 - 2 : y1 + 3, z1 - 2 : z1 + 3]), [3, 5, 5], nfase
                )[0:2, 1:-1, 1:-1]
    elif x1 == 1:
        if y1 == 0:
            if z1 == 0:
                bord[0:3, 0:2, 0:2] = borderoptsf3D(
                    (sg[0:4, 0:3, 0:3]), [4, 3, 3], nfase
                )[0:3, 0:2, 0:2]
            elif z1 == 1:
                bord[0:3, 0:2, 0:3] = borderoptsf3D(
                    (sg[0:4, 0:3, 0:4]), [4, 3, 4], nfase
                )[0:3, 0:2, 0:3]
            elif z1 == Z - 2:
                bord[0:3, 0:2, -3:] = borderoptsf3D(
                    (sg[0:4, 0:3, -4:]), [4, 3, 4], nfase
                )[0:3, 0:2, 1:]
            elif z1 == Z - 1:
                bord[0:3, 0:2, -2:] = borderoptsf3D(
                    (sg[0:4, 0:3, -3:]), [4, 3, 3], nfase
                )[0:3, 0:2, 1:]
            else:
                bord[0:3, 0:2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:4, 0:3, z1 - 2 : z1 + 3]), [4, 3, 5], nfase
                )[0:3, 0:2, 1:-1]
        elif y1 == 1:
            if z1 == 0:
                bord[0:3, 0:3, 0:2] = borderoptsf3D(
                    (sg[0:4, 0:4, 0:3]), [4, 4, 3], nfase
                )[0:3, 0:3, 0:2]
            elif z1 == 1:
                bord[0:3, 0:3, 0:3] = borderoptsf3D(
                    (sg[0:4, 0:4, 0:4]), [4, 4, 4], nfase
                )[0:3, 0:3, 0:3]
            elif z1 == Z - 2:
                bord[0:3, 0:3, -3:] = borderoptsf3D(
                    (sg[0:4, 0:4, -4:]), [4, 4, 4], nfase
                )[0:3, 0:3, 1:]
            elif z1 == Z - 1:
                bord[0:3, 0:3, -2:] = borderoptsf3D(
                    (sg[0:4, 0:4, -3:]), [4, 4, 3], nfase
                )[0:3, 0:3, 1:]
            else:
                bord[0:3, 0:3, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:4, 0:4, z1 - 2 : z1 + 3]), [4, 4, 5], nfase
                )[0:3, 0:3, 1:-1]
        elif y1 == Y - 2:
            if z1 == 0:
                bord[0:3, -3:, 0:2] = borderoptsf3D(
                    (sg[0:4, -4:, 0:3]), [4, 4, 3], nfase
                )[0:3, 1:, 0:2]
            elif z1 == 1:
                bord[0:3, -3:, 0:3] = borderoptsf3D(
                    (sg[0:4, -4:, 0:4]), [4, 4, 4], nfase
                )[0:3, 1:, 0:3]
            elif z1 == Z - 2:
                bord[0:3, -3:, -3:] = borderoptsf3D(
                    (sg[0:4, -4:, -4:]), [4, 4, 4], nfase
                )[0:3, 1:, 1:]
            elif z1 == Z - 1:
                bord[0:3, -3:, -2:] = borderoptsf3D(
                    (sg[0:4, -4:, -3:]), [4, 4, 3], nfase
                )[0:3, 1:, 1:]
            else:
                bord[0:3, -3:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:4, -4:, z1 - 2 : z1 + 3]), [4, 4, 5], nfase
                )[0:3, 1:, 1:-1]
        elif y1 == Y - 1:
            if z1 == 0:
                bord[0:3, -2:, 0:2] = borderoptsf3D(
                    (sg[0:4, -3:, 0:3]), [4, 3, 3], nfase
                )[0:3, 1:, 0:2]
            elif z1 == 1:
                bord[0:3, -2:, 0:3] = borderoptsf3D(
                    (sg[0:4, -3:, 0:4]), [4, 3, 4], nfase
                )[0:3, 1:, 0:3]
            elif z1 == Z - 2:
                bord[0:3, -2:, -3:] = borderoptsf3D(
                    (sg[0:4:, -3:, -4:]), [4, 3, 4], nfase
                )[0:3, 1:, 1:]
            elif z1 == Z - 1:
                bord[0:3, -2:, -2:] = borderoptsf3D(
                    (sg[0:4:, -3:, -3:]), [4, 3, 3], nfase
                )[0:3, 1:, 1:]
            else:
                bord[0:3, -2:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:4, -3:, z1 - 2 : z1 + 3]), [4, 3, 5], nfase
                )[0:3, 1:, 1:-1]
        else:
            if z1 == 0:
                bord[0:3, y1 - 1 : y1 + 2, 0:2] = borderoptsf3D(
                    (sg[0:4, y1 - 2 : y1 + 3, 0:3]), [4, 5, 3], nfase
                )[0:3, 1:-1, 0:2]
            elif z1 == 1:
                bord[0:3, y1 - 1 : y1 + 2, 0:3] = borderoptsf3D(
                    (sg[0:4, y1 - 2 : y1 + 3, 0:4]), [4, 5, 4], nfase
                )[0:3, 1:-1, 0:3]
            elif z1 == Z - 2:
                bord[0:3, y1 - 1 : y1 + 2, -3:] = borderoptsf3D(
                    (sg[0:4, y1 - 2 : y1 + 3, -4:]), [4, 5, 4], nfase
                )[0:3, 1:-1, 1:]
            elif z1 == Z - 1:
                bord[0:3, y1 - 1 : y1 + 2, -2:] = borderoptsf3D(
                    (sg[0:4, y1 - 2 : y1 + 3, -3:]), [4, 5, 3], nfase
                )[0:3, 1:-1, 1:]
            else:
                bord[0:3, y1 - 1 : y1 + 2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[0:4, y1 - 2 : y1 + 3, z1 - 2 : z1 + 3]), [4, 5, 5], nfase
                )[0:3, 1:-1, 1:-1]
    elif x1 == X - 2:
        if y1 == 0:
            if z1 == 0:
                bord[-3:, 0:2, 0:2] = borderoptsf3D(
                    (sg[-4:, 0:3, 0:3]), [4, 3, 3], nfase
                )[1:, 0:2, 0:2]
            elif z1 == 1:
                bord[-3:, 0:2, 0:3] = borderoptsf3D(
                    (sg[-4:, 0:3, 0:4]), [4, 3, 4], nfase
                )[1:, 0:2, 0:3]
            elif z1 == Z - 2:
                bord[-3:, 0:2, -3:] = borderoptsf3D(
                    (sg[-4:, 0:3, -4:]), [4, 3, 4], nfase
                )[1:, 0:2, 1:]
            elif z1 == Z - 1:
                bord[-3:, 0:2, -2:] = borderoptsf3D(
                    (sg[-4:, 0:3, -3:]), [4, 3, 3], nfase
                )[1:, 0:2, 1:]
            else:
                bord[-3:, 0:2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-4:, 0:3, z1 - 2 : z1 + 3]), [4, 3, 5], nfase
                )[1:, 0:2, 1:-1]
        elif y1 == 1:
            if z1 == 0:
                bord[-3:, 0:3, 0:2] = borderoptsf3D(
                    (sg[-4:, 0:4, 0:3]), [4, 4, 3], nfase
                )[1:, 0:3, 0:2]
            elif z1 == 1:
                bord[-3:, 0:3, 0:3] = borderoptsf3D(
                    (sg[-4:, 0:4, 0:4]), [4, 4, 4], nfase
                )[1:, 0:3, 0:3]
            elif z1 == Z - 2:
                bord[-3:, 0:3, -3:] = borderoptsf3D(
                    (sg[-4:, 0:4, -4:]), [4, 4, 4], nfase
                )[1:, 0:3, 1:]
            elif z1 == Z - 1:
                bord[-3:, 0:3, -2:] = borderoptsf3D(
                    (sg[-4:, 0:4, -3:]), [4, 4, 3], nfase
                )[1:, 0:3, 1:]
            else:
                bord[-3:, 0:3, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-4:, 0:4, z1 - 2 : z1 + 3]), [4, 4, 5], nfase
                )[1:, 0:3, 1:-1]
        elif y1 == Y - 2:
            if z1 == 0:
                bord[-3:, -3:, 0:2] = borderoptsf3D(
                    (sg[-4:, -4:, 0:3]), [4, 4, 3], nfase
                )[1:, 1:, 0:2]
            elif z1 == 1:
                bord[-3:, -3:, 0:3] = borderoptsf3D(
                    (sg[-4:, -4:, 0:4]), [4, 4, 4], nfase
                )[1:, 1:, 0:3]
            elif z1 == Z - 2:
                bord[-3:, -3:, -3:] = borderoptsf3D(
                    (sg[-4:, -4:, -4:]), [4, 4, 4], nfase
                )[1:, 1:, 1:]
            elif z1 == Z - 1:
                bord[-3:, -3:, -2:] = borderoptsf3D(
                    (sg[-4:, -4:, -3:]), [4, 4, 3], nfase
                )[1:, 1:, 1:]
            else:
                bord[-3:, -3:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-4:, -4:, z1 - 2 : z1 + 3]), [4, 4, 5], nfase
                )[1:, 1:, 1:-1]
        elif y1 == Y - 1:
            if z1 == 0:
                bord[-3:, -2:, 0:2] = borderoptsf3D(
                    (sg[-4:, -3:, 0:3]), [4, 3, 3], nfase
                )[1:, 1:, 0:2]
            elif z1 == 1:
                bord[-3:, -2:, 0:3] = borderoptsf3D(
                    (sg[-4:, -3:, 0:4]), [4, 3, 4], nfase
                )[1:, 1:, 0:3]
            elif z1 == Z - 2:
                bord[-3:, -2:, -3:] = borderoptsf3D(
                    (sg[-4:, -3:, -4:]), [4, 3, 4], nfase
                )[1:, 1:, 1:]
            elif z1 == Z - 1:
                bord[-3:, -2:, -2:] = borderoptsf3D(
                    (sg[-4:, -3:, -3:]), [4, 3, 3], nfase
                )[1:, 1:, 1:]
            else:
                bord[-3:, -2:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-4:, -3:, z1 - 2 : z1 + 3]), [4, 3, 5], nfase
                )[1:, 1:, 1:-1]
        else:
            if z1 == 0:
                bord[-3:, y1 - 1 : y1 + 2, 0:2] = borderoptsf3D(
                    (sg[-4:, y1 - 2 : y1 + 3, 0:3]), [4, 5, 3], nfase
                )[1:, 1:-1, 0:2]
            elif z1 == 1:
                bord[-3:, y1 - 1 : y1 + 2, 0:3] = borderoptsf3D(
                    (sg[-4:, y1 - 2 : y1 + 3, 0:4]), [4, 5, 4], nfase
                )[1:, 1:-1, 0:3]
            elif z1 == Z - 2:
                bord[-3:, y1 - 1 : y1 + 2, -3:] = borderoptsf3D(
                    (sg[-4:, y1 - 2 : y1 + 3, -4:]), [4, 5, 4], nfase
                )[1:, 1:-1, 1:]
            elif z1 == Z - 1:
                bord[-3:, y1 - 1 : y1 + 2, -2:] = borderoptsf3D(
                    (sg[-4:, y1 - 2 : y1 + 3, -3:]), [4, 5, 3], nfase
                )[1:, 1:-1, 1:]
            else:
                bord[-3:, y1 - 1 : y1 + 2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-4:, y1 - 2 : y1 + 3, z1 - 2 : z1 + 3]), [4, 5, 5], nfase
                )[1:, 1:-1, 1:-1]
    elif x1 == X - 1:
        if y1 == 0:
            if z1 == 0:
                bord[-2:, 0:2, 0:2] = borderoptsf3D(
                    (sg[-3:, 0:3, 0:3]), [3, 3, 3], nfase
                )[1:, 0:2, 0:2]
            elif z1 == 1:
                bord[-2:, 0:2, 0:3] = borderoptsf3D(
                    (sg[-3:, 0:3, 0:4]), [3, 3, 4], nfase
                )[1:, 0:2, 0:3]
            elif z1 == Z - 2:
                bord[-2:, 0:2, -3:] = borderoptsf3D(
                    (sg[-3:, 0:3, -4:]), [3, 3, 4], nfase
                )[1:, 0:2, 1:]
            elif z1 == Z - 1:
                bord[-2:, 0:2, -2:] = borderoptsf3D(
                    (sg[-3:, 0:3, -3:]), [3, 3, 3], nfase
                )[1:, 0:2, 1:]
            else:
                bord[-2:, 0:2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-3:, 0:3, z1 - 2 : z1 + 3]), [3, 3, 5], nfase
                )[1:, 0:2, 1:-1]
        elif y1 == 1:
            if z1 == 0:
                bord[-2:, 0:3, 0:2] = borderoptsf3D(
                    (sg[-3:, 0:4, 0:3]), [3, 4, 3], nfase
                )[1:, 0:3, 0:2]
            elif z1 == 1:
                bord[-2:, 0:3, 0:3] = borderoptsf3D(
                    (sg[-3:, 0:4, 0:4]), [3, 4, 4], nfase
                )[1:, 0:3, 0:3]
            elif z1 == Z - 2:
                bord[-2:, 0:3, -3:] = borderoptsf3D(
                    (sg[-3:, 0:4, -4:]), [3, 4, 4], nfase
                )[1:, 0:3, 1:]
            elif z1 == Z - 1:
                bord[-2:, 0:3, -2:] = borderoptsf3D(
                    (sg[-3:, 0:4, -3:]), [3, 4, 3], nfase
                )[1:, 0:3, 1:]
            else:
                bord[-2:, 0:3, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-3:, 0:4, z1 - 2 : z1 + 3]), [3, 4, 5], nfase
                )[1:, 0:3, 1:-1]
        elif y1 == Y - 2:
            if z1 == 0:
                bord[-2:, -3:, 0:2] = borderoptsf3D(
                    (sg[-3:, -4:, 0:3]), [3, 4, 3], nfase
                )[1:, 1:, 0:2]
            elif z1 == 1:
                bord[-2:, -3:, 0:3] = borderoptsf3D(
                    (sg[-3:, -4:, 0:4]), [3, 4, 4], nfase
                )[1:, 1:, 0:3]
            elif z1 == Z - 2:
                bord[-2:, -3:, -3:] = borderoptsf3D(
                    (sg[-3:, -4:, -4:]), [3, 4, 4], nfase
                )[1:, 1:, 1:]
            elif z1 == Z - 1:
                bord[-2:, -3:, -2:] = borderoptsf3D(
                    (sg[-3:, -4:, -3:]), [3, 4, 3], nfase
                )[1:, 1:, 1:]
            else:
                bord[-2:, -3:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-3:, -4:, z1 - 2 : z1 + 3]), [3, 4, 5], nfase
                )[1:, 1:, 1:-1]
        elif y1 == Y - 1:
            if z1 == 0:
                bord[-2:, -2:, 0:2] = borderoptsf3D(
                    (sg[-3:, -3:, 0:3]), [3, 3, 3], nfase
                )[1:, 1:, 0:2]
            elif z1 == 1:
                bord[-2:, -2:, 0:3] = borderoptsf3D(
                    (sg[-3:, -3:, 0:4]), [3, 3, 4], nfase
                )[1:, 1:, 0:3]
            elif z1 == Z - 2:
                bord[-2:, -2:, -3:] = borderoptsf3D(
                    (sg[-3:, -3:, -4:]), [3, 3, 4], nfase
                )[1:, 1:, 1:]
            elif z1 == Z - 1:
                bord[-2:, -2:, -2:] = borderoptsf3D(
                    (sg[-3:, -3:, -3:]), [3, 3, 3], nfase
                )[1:, 1:, 1:]
            else:
                bord[-2:, -2:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-3:, -3:, z1 - 2 : z1 + 3]), [3, 3, 5], nfase
                )[1:, 1:, 1:-1]
        else:
            if z1 == 0:
                bord[-2:, y1 - 1 : y1 + 2, 0:2] = borderoptsf3D(
                    (sg[-3:, y1 - 2 : y1 + 3, 0:3]), [3, 5, 3], nfase
                )[1:, 1:-1, 0:2]
            elif z1 == 1:
                bord[-2:, y1 - 1 : y1 + 2, 0:3] = borderoptsf3D(
                    (sg[-3:, y1 - 2 : y1 + 3, 0:4]), [3, 5, 4], nfase
                )[1:, 1:-1, 0:3]
            elif z1 == Z - 2:
                bord[-2:, y1 - 1 : y1 + 2, -3:] = borderoptsf3D(
                    (sg[-3:, y1 - 2 : y1 + 3, -4:]), [3, 5, 4], nfase
                )[1:, 1:-1, 1:]
            elif z1 == Z - 1:
                bord[-2:, y1 - 1 : y1 + 2, -2:] = borderoptsf3D(
                    (sg[-3:, y1 - 2 : y1 + 3, -3:]), [3, 5, 3], nfase
                )[1:, 1:-1, 1:]
            else:
                bord[-2:, y1 - 1 : y1 + 2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[-3:, y1 - 2 : y1 + 3, z1 - 2 : z1 + 3]), [3, 5, 5], nfase
                )[1:, 1:-1, 1:-1]
    else:
        if y1 == 0:
            if z1 == 0:
                bord[x1 - 1 : x1 + 2, 0:2, 0:2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:3, 0:3]), [5, 3, 3], nfase
                )[1:-1, 0:2, 0:2]
            elif z1 == 1:
                bord[x1 - 1 : x1 + 2, 0:2, 0:3] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:3, 0:4]), [5, 3, 4], nfase
                )[1:-1, 0:2, 0:3]
            elif z1 == Z - 2:
                bord[x1 - 1 : x1 + 2, 0:2, -3:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:3, -4:]), [5, 3, 4], nfase
                )[1:-1, 0:2, 1:]
            elif z1 == Z - 1:
                bord[x1 - 1 : x1 + 2, 0:2, -2:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:3, -3:]), [5, 3, 3], nfase
                )[1:-1, 0:2, 1:]
            else:
                bord[x1 - 1 : x1 + 2, 0:2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:3, z1 - 2 : z1 + 3]), [5, 3, 5], nfase
                )[1:-1, 0:2, 1:-1]
        elif y1 == 1:
            if z1 == 0:
                bord[x1 - 1 : x1 + 2, 0:3, 0:2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:4, 0:3]), [5, 4, 3], nfase
                )[1:-1, 0:3, 0:2]
            elif z1 == 1:
                bord[x1 - 1 : x1 + 2, 0:3, 0:3] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:4, 0:4]), [5, 4, 4], nfase
                )[1:-1, 0:3, 0:3]
            elif z1 == Z - 2:
                bord[x1 - 1 : x1 + 2, 0:3, -3:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:4, -4:]), [5, 4, 4], nfase
                )[1:-1, 0:3, 1:]
            elif z1 == Z - 1:
                bord[x1 - 1 : x1 + 2, 0:3, -2:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:4, -3:]), [5, 4, 3], nfase
                )[1:-1, 0:3, 1:]
            else:
                bord[x1 - 1 : x1 + 2, 0:3, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, 0:4, z1 - 2 : z1 + 3]), [5, 4, 5], nfase
                )[1:-1, 0:3, 1:-1]
        elif y1 == Y - 2:
            if z1 == 0:
                bord[x1 - 1 : x1 + 2, -3:, 0:2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -4:, 0:3]), [5, 4, 3], nfase
                )[1:-1, 1:, 0:2]
            elif z1 == 1:
                bord[x1 - 1 : x1 + 2, -3:, 0:3] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -4:, 0:4]), [5, 4, 4], nfase
                )[1:-1, 1:, 0:3]
            elif z1 == Z - 2:
                bord[x1 - 1 : x1 + 2, -3:, -3:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -4:, -4:]), [5, 4, 4], nfase
                )[1:-1, 1:, 1:]
            elif z1 == Z - 1:
                bord[x1 - 1 : x1 + 2, -3:, -2:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -4:, -3:]), [5, 4, 3], nfase
                )[1:-1, 1:, 1:]
            else:
                bord[x1 - 1 : x1 + 2, -3:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -4:, z1 - 2 : z1 + 3]), [5, 4, 5], nfase
                )[1:-1, 1:, 1:-1]
        elif y1 == Y - 1:
            if z1 == 0:
                bord[x1 - 1 : x1 + 2, -2:, 0:2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -3:, 0:3]), [5, 3, 3], nfase
                )[1:-1, 1:, 0:2]
            elif z1 == 1:
                bord[x1 - 1 : x1 + 2, -2:, 0:3] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -3:, 0:4]), [5, 3, 4], nfase
                )[1:-1, 1:, 0:3]
            elif z1 == Z - 2:
                bord[x1 - 1 : x1 + 2, -2:, -3:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -3:, -4:]), [5, 3, 4], nfase
                )[1:-1, 1:, 1:]
            elif z1 == Z - 1:
                bord[x1 - 1 : x1 + 2, -2:, -2:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -3:, -3:]), [5, 3, 3], nfase
                )[1:-1, 1:, 1:]
            else:

                bord[x1 - 1 : x1 + 2, -2:, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, -3:, z1 - 2 : z1 + 3]), [5, 3, 5], nfase
                )[1:-1, 1:, 1:-1]
        else:
            if z1 == 0:
                bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2, 0:2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3, 0:3]), [5, 5, 3], nfase
                )[1:-1, 1:-1, 0:2]
            elif z1 == 1:
                bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2, 0:3] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3, 0:4]), [5, 5, 4], nfase
                )[1:-1, 1:-1, 0:3]
            elif z1 == Z - 2:
                bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2, -3:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3, -4:]), [5, 5, 4], nfase
                )[1:-1, 1:-1, 1:]
            elif z1 == Z - 1:
                bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2, -2:] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3, -3:]), [5, 5, 3], nfase
                )[1:-1, 1:-1, 1:]
            else:

                bord[x1 - 1 : x1 + 2, y1 - 1 : y1 + 2, z1 - 1 : z1 + 2] = borderoptsf3D(
                    (sg[x1 - 2 : x1 + 3, y1 - 2 : y1 + 3, z1 - 2 : z1 + 3]),
                    [5, 5, 5],
                    nfase,
                )[1:-1, 1:-1, 1:-1]
    if x2 == 0:
        if y2 == 0:
            if z2 == 0:
                ...
                bord[0:2, 0:2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:3, 0:3, z2 - 2 : z2 + 3]), [3, 3, 5], nfase
                )[0:2, 0:2, 1:-1]
        elif y2 == 1:
            if z2 == 0:
                bord[0:2, 0:3, 0:2] = borderoptsf3D(
                ...
                bord[0:2, 0:3, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:3, 0:4, z2 - 2 : z2 + 3]), [3, 4, 5], nfase
                )[0:2, 0:3, 1:-1]
        elif y2 == Y - 2:
            if z2 == 0:
                bord[0:2, -3:, 0:2] = borderoptsf3D(
                ...
                bord[0:2, -3:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:3, -4:, z2 - 2 : z2 + 3]), [3, 4, 5], nfase
                )[0:2, 1:, 1:-1]
        elif y2 == Y - 1:
            if z2 == 0:
                bord[0:2, -2:, 0:2] = borderoptsf3D(
                ...
                bord[0:2, -2:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:3, -3:, z2 - 2 : z2 + 3]), [3, 3, 5], nfase
                )[0:2, 1:, 1:-1]
        else:
            if z2 == 0:
                bord[0:2, y2 - 1 : y2 + 2, 0:2] = borderoptsf3D(
                    (sg[0:3, y2 - 2 : y2 + 3, 0:3]), [3, 5, 3], nfase
                )[0:2, 1:-1, 0:2]
            elif z2 == 1:
                bord[0:2, y2 - 1 : y2 + 2, 0:3] = borderoptsf3D(
                    (sg[0:3, y2 - 2 : y2 + 3, 0:4]), [3, 5, 4], nfase
                )[0:2, 1:-1, 0:3]
            elif z2 == Z - 2:
                bord[0:2, y2 - 1 : y2 + 2, -3:] = borderoptsf3D(
                    (sg[0:3, y2 - 2 : y2 + 3, -4:]), [3, 5, 4], nfase
                )[0:2, 1:-1, 1:]
            elif z2 == Z - 1:
                bord[0:2, y2 - 1 : y2 + 2, -2:] = borderoptsf3D(
                    (sg[0:3, y2 - 2 : y2 + 3, -3:]), [3, 5, 3], nfase
                )[0:2, 1:-1, 1:]
            else:
                bord[0:2, y2 - 1 : y2 + 2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:3, y2 - 2 : y2 + 3, z2 - 2 : z2 + 3]), [3, 5, 5], nfase
                )[0:2, 1:-1, 1:-1]
    elif x2 == 1:
        if y2 == 0:
            if z2 == 0:
                ...
                bord[0:3, 0:2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:4, 0:3, z2 - 2 : z2 + 3]), [4, 3, 5], nfase
                )[0:3, 0:2, 1:-1]
        elif y2 == 1:
            if z2 == 0:
                bord[0:3, 0:3, 0:2] = borderoptsf3D(
                ...
                bord[0:3, 0:3, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:4, 0:4, z2 - 2 : z2 + 3]), [4, 4, 5], nfase
                )[0:3, 0:3, 1:-1]
        elif y2 == Y - 2:
            if z2 == 0:
                bord[0:3, -3:, 0:2] = borderoptsf3D(
                ...
                bord[0:3, -3:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:4, -4:, z2 - 2 : z2 + 3]), [4, 4, 5], nfase
                )[0:3, 1:, 1:-1]
        elif y2 == Y - 1:
            if z2 == 0:
                bord[0:3, -2:, 0:2] = borderoptsf3D(
                ...
                bord[0:3, -2:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:4, -3:, z2 - 2 : z2 + 3]), [4, 3, 5], nfase
                )[0:3, 1:, 1:-1]
        else:
            if z2 == 0:
                bord[0:3, y2 - 1 : y2 + 2, 0:2] = borderoptsf3D(
                    (sg[0:4, y2 - 2 : y2 + 3, 0:3]), [4, 5, 3], nfase
                )[0:3, 1:-1, 0:2]
            elif z2 == 1:
                bord[0:3, y2 - 1 : y2 + 2, 0:3] = borderoptsf3D(
                    (sg[0:4, y2 - 2 : y2 + 3, 0:4]), [4, 5, 4], nfase
                )[0:3, 1:-1, 0:3]
            elif z2 == Z - 2:
                bord[0:3, y2 - 1 : y2 + 2, -3:] = borderoptsf3D(
                    (sg[0:4, y2 - 2 : y2 + 3, -4:]), [4, 5, 4], nfase
                )[0:3, 1:-1, 1:]
            elif z2 == Z - 1:
                bord[0:3, y2 - 1 : y2 + 2, -2:] = borderoptsf3D(
                    (sg[0:4, y2 - 2 : y2 + 3, -3:]), [4, 5, 3], nfase
                )[0:3, 1:-1, 1:]
            else:
                bord[0:3, y2 - 1 : y2 + 2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[0:4, y2 - 2 : y2 + 3, z2 - 2 : z2 + 3]), [4, 5, 5], nfase
                )[0:3, 1:-1, 1:-1]
    elif x2 == X - 2:
        if y2 == 0:
            if z2 == 0:
                ...
                bord[-3:, 0:2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-4:, 0:3, z2 - 2 : z2 + 3]), [4, 3, 5], nfase
                )[1:, 0:2, 1:-1]
        elif y2 == 1:
            if z2 == 0:
                bord[-3:, 0:3, 0:2] = borderoptsf3D(
                ...
                bord[-3:, 0:3, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-4:, 0:4, z2 - 2 : z2 + 3]), [4, 4, 5], nfase
                )[1:, 0:3, 1:-1]
        elif y2 == Y - 2:
            if z2 == 0:
                bord[-3:, -3:, 0:2] = borderoptsf3D(
                ...
                bord[-3:, -3:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-4:, -4:, z2 - 2 : z2 + 3]), [4, 4, 5], nfase
                )[1:, 1:, 1:-1]
        elif y2 == Y - 1:
            if z2 == 0:
                bord[-3:, -2:, 0:2] = borderoptsf3D(
                ...
                bord[-3:, -2:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-4:, -3:, z2 - 2 : z2 + 3]), [4, 3, 5], nfase
                )[1:, 1:, 1:-1]
        else:
            if z2 == 0:
                bord[-3:, y2 - 1 : y2 + 2, 0:2] = borderoptsf3D(
                    (sg[-4:, y2 - 2 : y2 + 3, 0:3]), [4, 5, 3], nfase
                )[1:, 1:-1, 0:2]
            elif z2 == 1:
                bord[-3:, y2 - 1 : y2 + 2, 0:3] = borderoptsf3D(
                    (sg[-4:, y2 - 2 : y2 + 3, 0:4]), [4, 5, 4], nfase
                )[1:, 1:-1, 0:3]
            elif z2 == Z - 2:
                bord[-3:, y2 - 1 : y2 + 2, -3:] = borderoptsf3D(
                    (sg[-4:, y2 - 2 : y2 + 3, -4:]), [4, 5, 4], nfase
                )[1:, 1:-1, 1:]
            elif z2 == Z - 1:
                bord[-3:, y2 - 1 : y2 + 2, -2:] = borderoptsf3D(
                    (sg[-4:, y2 - 2 : y2 + 3, -3:]), [4, 5, 3], nfase
                )[1:, 1:-1, 1:]
            else:
                bord[-3:, y2 - 1 : y2 + 2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-4:, y2 - 2 : y2 + 3, z2 - 2 : z2 + 3]), [4, 5, 5], nfase
                )[1:, 1:-1, 1:-1]
    elif x2 == X - 1:
        if y2 == 0:
            if z2 == 0:
                ...
                bord[-2:, 0:2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-3:, 0:3, z2 - 2 : z2 + 3]), [3, 3, 5], nfase
                )[1:, 0:2, 1:-1]
        elif y2 == 1:
            if z2 == 0:
                bord[-2:, 0:3, 0:2] = borderoptsf3D(
                ...
                bord[-2:, 0:3, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-3:, 0:4, z2 - 2 : z2 + 3]), [3, 4, 5], nfase
                )[1:, 0:3, 1:-1]
        elif y2 == Y - 2:
            if z2 == 0:
                bord[-2:, -3:, 0:2] = borderoptsf3D(
                ...
                bord[-2:, -3:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-3:, -4:, z2 - 2 : z2 + 3]), [3, 4, 5], nfase
                )[1:, 1:, 1:-1]
        elif y2 == Y - 1:
            if z2 == 0:
                bord[-2:, -2:, 0:2] = borderoptsf3D(
                ...
                bord[-2:, -2:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-3:, -3:, z2 - 2 : z2 + 3]), [3, 3, 5], nfase
                )[1:, 1:, 1:-1]
        else:
            if z2 == 0:
                bord[-2:, y2 - 1 : y2 + 2, 0:2] = borderoptsf3D(
                    (sg[-3:, y2 - 2 : y2 + 3, 0:3]), [3, 5, 3], nfase
                )[1:, 1:-1, 0:2]
            elif z2 == 1:
                bord[-2:, y2 - 1 : y2 + 2, 0:3] = borderoptsf3D(
                    (sg[-3:, y2 - 2 : y2 + 3, 0:4]), [3, 5, 4], nfase
                )[1:, 1:-1, 0:3]
            elif z2 == Z - 2:
                bord[-2:, y2 - 1 : y2 + 2, -3:] = borderoptsf3D(
                    (sg[-3:, y2 - 2 : y2 + 3, -4:]), [3, 5, 4], nfase
                )[1:, 1:-1, 1:]
            elif z2 == Z - 1:
                bord[-2:, y2 - 1 : y2 + 2, -2:] = borderoptsf3D(
                    (sg[-3:, y2 - 2 : y2 + 3, -3:]), [3, 5, 3], nfase
                )[1:, 1:-1, 1:]
            else:
                bord[-2:, y2 - 1 : y2 + 2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[-3:, y2 - 2 : y2 + 3, z2 - 2 : z2 + 3]), [3, 5, 5], nfase
                )[1:, 1:-1, 1:-1]
    else:
        if y2 == 0:
            if z2 == 0:
                bord[x2 - 1 : x2 + 2, 0:2, 0:2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:3, 0:3]), [5, 3, 3], nfase
                )[1:-1, 0:2, 0:2]
            elif z2 == 1:
                bord[x2 - 1 : x2 + 2, 0:2, 0:3] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:3, 0:4]), [5, 3, 4], nfase
                )[1:-1, 0:2, 0:3]
            elif z2 == Z - 2:
                bord[x2 - 1 : x2 + 2, 0:2, -3:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:3, -4:]), [5, 3, 4], nfase
                )[1:-1, 0:2, 1:]
            elif z2 == Z - 1:
                bord[x2 - 1 : x2 + 2, 0:2, -2:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:3, -3:]), [5, 3, 3], nfase
                )[1:-1, 0:2, 1:]
            else:
                bord[x2 - 1 : x2 + 2, 0:2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:3, z2 - 2 : z2 + 3]), [5, 3, 5], nfase
                )[1:-1, 0:2, 1:-1]
        elif y2 == 1:
            if z2 == 0:
                bord[x2 - 1 : x2 + 2, 0:3, 0:2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:4, 0:3]), [5, 4, 3], nfase
                )[1:-1, 0:3, 0:2]
            elif z2 == 1:
                bord[x2 - 1 : x2 + 2, 0:3, 0:3] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:4, 0:4]), [5, 4, 4], nfase
                )[1:-1, 0:3, 0:3]
            elif z2 == Z - 2:
                bord[x2 - 1 : x2 + 2, 0:3, -3:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:4, -4:]), [5, 4, 4], nfase
                )[1:-1, 0:3, 1:]
            elif z2 == Z - 1:
                bord[x2 - 1 : x2 + 2, 0:3, -2:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:4, -3:]), [5, 4, 3], nfase
                )[1:-1, 0:3, 1:]
            else:
                bord[x2 - 1 : x2 + 2, 0:3, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, 0:4, z2 - 2 : z2 + 3]), [5, 4, 5], nfase
                )[1:-1, 0:3, 1:-1]
        elif y2 == Y - 2:
            if z2 == 0:
                bord[x2 - 1 : x2 + 2, -3:, 0:2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -4:, 0:3]), [5, 4, 3], nfase
                )[1:-1, 1:, 0:2]
            elif z2 == 1:
                bord[x2 - 1 : x2 + 2, -3:, 0:3] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -4:, 0:4]), [5, 4, 4], nfase
                )[1:-1, 1:, 0:3]
            elif z2 == Z - 2:
                bord[x2 - 1 : x2 + 2, -3:, -3:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -4:, -4:]), [5, 4, 4], nfase
                )[1:-1, 1:, 1:]
            elif z2 == Z - 1:
                bord[x2 - 1 : x2 + 2, -3:, -2:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -4:, -3:]), [5, 4, 3], nfase
                )[1:-1, 1:, 1:]
            else:
                bord[x2 - 1 : x2 + 2, -3:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -4:, z2 - 2 : z2 + 3]), [5, 4, 5], nfase
                )[1:-1, 1:, 1:-1]
        elif y2 == Y - 1:
            if z2 == 0:
                bord[x2 - 1 : x2 + 2, -2:, 0:2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -3:, 0:3]), [5, 3, 3], nfase
                )[1:-1, 1:, 0:2]
            elif z2 == 1:
                bord[x2 - 1 : x2 + 2, -2:, 0:3] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -3:, 0:4]), [5, 3, 4], nfase
                )[1:-1, 1:, 0:3]
            elif z2 == Z - 2:
                bord[x2 - 1 : x2 + 2, -2:, -3:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -3:, -4:]), [5, 3, 4], nfase
                )[1:-1, 1:, 1:]
            elif z2 == Z - 1:
                bord[x2 - 1 : x2 + 2, -2:, -2:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -3:, -3:]), [5, 3, 3], nfase
                )[1:-1, 1:, 1:]
            else:
                bord[x2 - 1 : x2 + 2, -2:, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, -3:, z2 - 2 : z2 + 3]), [5, 3, 5], nfase
                )[1:-1, 1:, 1:-1]
        else:
            if z2 == 0:
                bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2, 0:2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3, 0:3]), [5, 5, 3], nfase
                )[1:-1, 1:-1, 0:2]
            elif z2 == 1:
                bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2, 0:3] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3, 0:4]), [5, 5, 4], nfase
                )[1:-1, 1:-1, 0:3]
            elif z2 == Z - 2:
                bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2, -3:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3, -4:]), [5, 5, 4], nfase
                )[1:-1, 1:-1, 1:]
            elif z2 == Z - 1:
                bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2, -2:] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3, -3:]), [5, 5, 3], nfase
                )[1:-1, 1:-1, 1:]
            else:
                bord[x2 - 1 : x2 + 2, y2 - 1 : y2 + 2, z2 - 1 : z2 + 2] = borderoptsf3D(
                    (sg[x2 - 2 : x2 + 3, y2 - 2 : y2 + 3, z2 - 2 : z2 + 3]),
                    [5, 5, 5],
                    nfase,
                )[1:-1, 1:-1, 1:-1]
...
def annstep_SF3d(
    ...
    if nrdir == 3:
        ...
        sf_newz0, sf_newz1 = same_facies_up(
            sg_oldz,
            sg_newz,
            sf_oldz0,
            sf_oldz1,
        ...
        dif_sf = np.sum(
            ...
            + (sftih1 / sftih1[0] - sf_newh1 / sf_newh1[0]) ** 2
            + (sftiv1 / sftiv1[0] - sf_newv1 / sf_newv1[0]) ** 2
            + (sftiz0 / sftiz0[0] - sf_newz0 / sf_newz0[0]) ** 2
            + (sftiz1 / sftiz1[0] - sf_newz1 / sf_newz1[0]) ** 2
        )
        ...
        sf_newxy0, sf_newxy1 = same_facies_up(
            sg_oldxy,
            sg_newxy,
            sf_oldxy0,
            sf_oldxy1,
        ...
        sf_newxz0, sf_newxz1 = same_facies_up(
            sg_oldxz,
            sg_newxz,
            sf_oldxz0,
            sf_oldxz1,
        ...
        sf_newyz0, sf_newyz1 = same_facies_up(
            sg_oldyz,
            sg_newyz,
            sf_oldyz0,
            sf_oldyz1,
        ...
        dif_sf = np.sum(
            ...
            (sftidxy0 / sftidxy0[0] - sf_newxy0 / sf_newxy0[0]) ** 2
            + (sftidxy1 / sftidxy1[0] - sf_newxy1 / sf_newxy1[0]) ** 2
            + (sftidxz0 / sftidxz0[0] - sf_newxz0 / sf_newxz0[0]) ** 2
            + (sftidxz1 / sftidxz1[0] - sf_newxz1 / sf_newxz1[0]) ** 2
            + (sftidyz0 / sftidyz0[0] - sf_newyz0 / sf_newyz0[0]) ** 2
            + (sftidyz1 / sftidyz1[0] - sf_newyz1 / sf_newyz1[0]) ** 2
        )
        return (
            dif_sf,
            sf_newh0,
        ...
        sf_newyx0, sf_newyx1 = same_facies_up(
            sg_oldyx,
            sg_newyx,
            sf_oldyx0,
            sf_oldyx1,
        ...
        sf_newzx0, sf_newzx1 = same_facies_up(
            sg_oldzx,
            sg_newzx,
            sf_oldzx0,
            sf_oldzx1,
        ...
        sf_newzy0, sf_newzy1 = same_facies_up(
            sg_oldzy,
            sg_newzy,
            sf_oldzy0,
            sf_oldzy1,
        ...
        dif_sf = (
            ...
            + np.sum(
                (sftidyx0 / sftidyx0[0] - sf_newyx0 / sf_newyx0[0]) ** 2
                + (sftidyx1 / sftidyx1[0] - sf_newyx1 / sf_newyx1[0]) ** 2
                + (sftidzx0 / sftidzx0[0] - sf_newzx0 / sf_newzx0[0]) ** 2
                + (sftidzx1 / sftidzx1[0] - sf_newzx1 / sf_newzx1[0]) ** 2
                + (sftidzy0 / sftidzy0[0] - sf_newzy0 / sf_newzy0[0]) ** 2
                + (sftidzy1 / sftidzy1[0] - sf_newzy1 / sf_newzy1[0]) ** 2
            )
        )
        return (
            dif_sf,
...
def annstep_LF3d(
    ...
    if nrdir == 3:
        lf_newh0 = linear_fct1_3D(
            sg_oldh, sg_newh, lf_oldh0, x, siztosub[0], target[0], length, "h", "u"
        )
        lf_newh1 = linear_fct1_3D(
            sg_oldh, sg_newh, lf_oldh1, x, siztosub[0], target[1], length, "h", "u"
        )
        lf_newv0 = linear_fct1_3D(
            sg_oldv, sg_newv, lf_oldv0, x, siztosub[1], target[0], length, "v", "u"
        )
        lf_newv1 = linear_fct1_3D(
            sg_oldv, sg_newv, lf_oldv1, x, siztosub[1], target[1], length, "v", "u"
        )
        lf_newz0 = linear_fct1_3D(
            sg_oldz, sg_newz, lf_oldz0, x, siztosub[1], target[0], length, "z", "u"
        )
        lf_newz1 = linear_fct1_3D(
            sg_oldz, sg_newz, lf_oldz1, x, siztosub[1], target[1], length, "z", "u"
        )
        ...
        dif_lf = np.sum(
            ...
            + (lf_newh1 / lf_newh1[0] - lf_tih1 / lf_tih1[0]) ** 2
            + (lf_newv1 / lf_newv1[0] - lf_tiv1 / lf_tiv1[0]) ** 2
            + (lf_newz0 / lf_newz0[0] - lf_tiz0 / lf_tiz0[0]) ** 2
            + (lf_newz1 / lf_newz1[0] - lf_tiz1 / lf_tiz1[0]) ** 2
        )
        ...
        dif_lf = np.sum(
            ...
            (lf_tidxy0 / lf_tidxy0[0] - lf_newxy0 / lf_newxy0[0]) ** 2
            + (lf_tidxy1 / lf_tidxy1[0] - lf_newxy1 / lf_newxy1[0]) ** 2
            + (lf_tidxz0 / lf_tidxz0[0] - lf_newxz0 / lf_newxz0[0]) ** 2
            + (lf_tidxz1 / lf_tidxz1[0] - lf_newxz1 / lf_newxz1[0]) ** 2
            + (lf_tidyz0 / lf_tidyz0[0] - lf_newyz0 / lf_newyz0[0]) ** 2
            + (lf_tidyz1 / lf_tidyz1[0] - lf_newyz1 / lf_newyz1[0]) ** 2
        )
        return (
            dif_lf,
            lf_newh0,
        ...
        dif_lf = (
            ...
            + np.sum(
                (lf_tidyx0 / lf_tidyx0[0] - lf_newyx0 / lf_newyx0[0]) ** 2
                + (lf_tidyx1 / lf_tidyx1[0] - lf_newyx1 / lf_newyx1[0]) ** 2
                + (lf_tidzx0 / lf_tidzx0[0] - lf_newzx0 / lf_newzx0[0]) ** 2
                + (lf_tidzx1 / lf_tidzx1[0] - lf_newzx1 / lf_newzx1[0]) ** 2
                + (lf_tidzy0 / lf_tidzy0[0] - lf_newzy0 / lf_newzy0[0]) ** 2
                + (lf_tidzy1 / lf_tidzy1[0] - lf_newzy1 / lf_newzy1[0]) ** 2
            )
        )
        return (
            dif_lf,
        ...
        if boundary == "c":
            ...
            if np.size(target) == 1:
                ...
                for i in range(1, length):
                    s_f[i] = np.count_nonzero(mt[:-i] + mt[i:] == 2 * target) / (
                        x - i
                    )  # the array is added to a shifted version of itself and compared to the desired sum value. The number of true statements divided by  the number of compared pairs gives the probability
            else:
                s_f = s_f = np.zeros((length, len(target)), dtype=float)
                for l in range(0, len(target)):
                    s_f[0, l] = np.count_nonzero(mt == target[l] / 2) / x
            ...
            if orientation == "h":
                ...
                row, col, posin = resh_diag2(
                    np.squeeze(mt[:, :, 0])
                )  # determing the order to reshape the array along its diagonal
                row = np.tile(row, Z)
                col = np.tile(col, Z)
                hei = np.repeat(np.arange(Z), X * Y)
...
def Smfinput3d(ti, target=[0, 1], length=100, nrdir=2, sgd=[], sg=[], npoins=[]):
    ...
    if len(tgd) == 2:
        if nrdir == 3:
            ...
            sf_oldh0 = same_facies3D(
                sg, target[0], length, orientation="h", boundary="u"
            )
            sf_oldh1 = same_facies3D(
                sg, target[1], length, orientation="h", boundary="u"
            )
            sf_oldv0 = same_facies3D(
                sg, target[0], length, orientation="v", boundary="u"
            )
            sf_oldv1 = same_facies3D(
                sg, target[1], length, orientation="v", boundary="u"
            )
            sf_oldz0 = same_facies3D(
                sg, target[0], length, orientation="z", boundary="u"
            )
            sf_oldz1 = same_facies3D(
                sg, target[1], length, orientation="z", boundary="u"
            )
            dold_sf = np.sum(
                (sftih0 / sftih0[0] - sf_oldh0 / sf_oldh0[0]) ** 2
                + (sftiv0 / sftiv0[0] - sf_oldv0 / sf_oldv0[0]) ** 2
                + (sftih1 / sftih1[0] - sf_oldh1 / sf_oldh1[0]) ** 2
                + (sftiv1 / sftiv1[0] - sf_oldv1 / sf_oldv1[0]) ** 2
                + (sftiz0 / sftiz0[0] - sf_oldz0 / sf_oldz0[0]) ** 2
                + (sftiz1 / sftiz1[0] - sf_oldz1 / sf_oldz1[0]) ** 2
            )
            return (
                sftih0,
                sftiv0,
            ...
            sf_oldxy0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dxy", boundary="u"
            )
            sf_oldxy1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dxy", boundary="u"
            )
            sf_oldxz0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dxz", boundary="u"
            )
            sf_oldxz1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dxz", boundary="u"
            )
            sf_oldyz0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dyz", boundary="u"
            )
            sf_oldyz1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dyz", boundary="u"
            )
            npoins = nboun(tgd, int(length / 1.414))
            row, col, posin = resh_diag2(ti)
            sftidxy0 = same_facies(
                ti,
                tar0,
                int(length / 1.414),
                orientation="d",
            ...
            dold_sf = np.sum(
                ...
                (sftidxy0 / sftidxy0[0] - sf_oldxy0 / sf_oldxy0[0]) ** 2
                + (sftidxy1 / sftidxy1[0] - sf_oldxy1 / sf_oldxy1[0]) ** 2
                + (sftidxz0 / sftidxz0[0] - sf_oldxz0 / sf_oldxz0[0]) ** 2
                + (sftidxz1 / sftidxz1[0] - sf_oldxz1 / sf_oldxz1[0]) ** 2
                + (sftidyz0 / sftidyz0[0] - sf_oldyz0 / sf_oldyz0[0]) ** 2
                + (sftidyz1 / sftidyz1[0] - sf_oldyz1 / sf_oldyz1[0]) ** 2
            )
            return (
                sftih0,
                sftiv0,
            ...
            sf_oldyx0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dyx", boundary="u"
            )
            sf_oldyx1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dyx", boundary="u"
            )
            sf_oldzx0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dzx", boundary="u"
            )
            sf_oldzx1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dzx", boundary="u"
            )
            sf_oldzy0 = same_facies3D(
                sg, target[0], int(length / 1.414), orientation="dzy", boundary="u"
            )
            sf_oldzy1 = same_facies3D(
                sg, target[1], int(length / 1.414), orientation="dzy", boundary="u"
            )
            npoins = nboun(tgd, int(length / 1.414))
            row, col, posin = resh_diag2(ti)
            sftidxy0 = same_facies(
            ...
            dold_sf = (
                ...
                + +np.sum(
                    (sftidyx0 / sftidyx0[0] - sf_oldyx0 / sf_oldyx0[0]) ** 2
                    + (sftidyx1 / sftidyx1[0] - sf_oldyx1 / sf_oldyx1[0]) ** 2
                    + (sftidzx0 / sftidzx0[0] - sf_oldzx0 / sf_oldzx0[0]) ** 2
                    + (sftidzx1 / sftidzx1[0] - sf_oldzx1 / sf_oldzx1[0]) ** 2
                    + (sftidzy0 / sftidzy0[0] - sf_oldzy0 / sf_oldzy0[0]) ** 2
                    + (sftidzy1 / sftidzy1[0] - sf_oldzy1 / sf_oldzy1[0]) ** 2
                )
            )
            return (
                sftih0,
...
def linear_fct3D(mt1, target=1, length=100, orientation="h", boundary="c"):
    ...
    if np.size(length) == 1:
        if boundary == "c":
            ...
            l_ine[0] = len(t) / x
            for i in range(1, length):
                t = (np.nonzero(t[1:] - t[:-1] == 1))[
                    0
                ]  # updating t to the new length,only consecutive entries stay
                T = np.shape(t)
                if T == ():
                    l_ine[i] == 1 / (x - i)
                    break
...
def linput3d(ti, target=[0, 1], length=100, nrdir=2, sgd=[], sg=[], npoins=[]):
    ...
    if len(tgd) == 2:
        ...
        if nrdir == 3:
            ...
            lf_oldh0 = linear_fct3D(
                sg, target[0], length, orientation="h", boundary="u"
            )
            lf_oldh1 = linear_fct3D(
                sg, target[1], length, orientation="h", boundary="u"
            )
            lf_oldv0 = linear_fct3D(
                sg, target[0], length, orientation="v", boundary="u"
            )
            lf_oldv1 = linear_fct3D(
                sg, target[1], length, orientation="v", boundary="u"
            )
            lf_oldz0 = linear_fct3D(
                sg, target[0], length, orientation="z", boundary="u"
            )
            lf_oldz1 = linear_fct3D(
                sg, target[1], length, orientation="z", boundary="u"
            )
            ...
            dold_lf = np.sum(
                ...
                + (lf_tiv0 / lf_tiv0[0] - lf_oldv0 / lf_oldv0[0]) ** 2
                + (lf_tiv1 / lf_tiv1[0] - lf_oldv1 / lf_oldv1[0]) ** 2
                + (lf_tiz0 / lf_tiz0[0] - lf_oldz0 / lf_oldz0[0]) ** 2
                + (lf_tiz1 / lf_tiz1[0] - lf_oldz1 / lf_oldz1[0]) ** 2
            )
            return (
                lf_tih0,
                lf_tiv0,
            ...
            lf_oldxy0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dxy", boundary="u"
            )
            lf_oldxy1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dxy", boundary="u"
            )
            lf_oldxz0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dxz", boundary="u"
            )
            lf_oldxz1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dxz", boundary="u"
            )
            lf_oldyz0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dyz", boundary="u"
            )
            lf_oldyz1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dyz", boundary="u"
            )
            dold_lf = np.sum(
                (lf_tih0 / lf_tih0[0] - lf_oldh0 / lf_oldh0[0]) ** 2
                ...
                (lf_tidxy0 / lf_tidxy0[0] - lf_oldxy0 / lf_oldxy0[0]) ** 2
                + (lf_tidxy1 / lf_tidxy1[0] - lf_oldxy1 / lf_oldxy1[0]) ** 2
                + (lf_tidxz0 / lf_tidxz0[0] - lf_oldxz0 / lf_oldxz0[0]) ** 2
                + (lf_tidxz1 / lf_tidxz1[0] - lf_oldxz1 / lf_oldxz1[0]) ** 2
                + (lf_tidyz0 / lf_tidyz0[0] - lf_oldyz0 / lf_oldyz0[0]) ** 2
                + (lf_tidyz1 / lf_tidyz1[0] - lf_oldyz1 / lf_oldyz1[0]) ** 2
            )
            return (
                lf_tih0,
                lf_tiv0,
            ...
            lf_oldyx0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dyx", boundary="u"
            )
            lf_oldyx1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dyx", boundary="u"
            )
            lf_oldzx0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dzx", boundary="u"
            )
            lf_oldzx1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dzx", boundary="u"
            )
            lf_oldzy0 = linear_fct3D(
                sg, target[0], int(length / 1.414), orientation="dzy", boundary="u"
            )
            lf_oldzy1 = linear_fct3D(
                sg, target[1], int(length / 1.414), orientation="dzy", boundary="u"
            )
            dold_lf = np.sum(
                (lf_tih0 / lf_tih0[0] - lf_oldh0 / lf_oldh0[0]) ** 2
                ...
                + (lf_tidyz0 / lf_tidyz0[0] - lf_oldyz0 / lf_oldyz0[0]) ** 2
                + (lf_tidyz1 / lf_tidyz1[0] - lf_oldyz1 / lf_oldyz1[0]) ** 2
                + (lf_tidyx0 / lf_tidyx0[0] - lf_oldyx0 / lf_oldyx0[0]) ** 2
                + (lf_tidyx1 / lf_tidyx1[0] - lf_oldyx1 / lf_oldyx1[0]) ** 2
                + (lf_tidzx0 / lf_tidzx0[0] - lf_oldzx0 / lf_oldzx0[0]) ** 2
                + (lf_tidzx1 / lf_tidzx1[0] - lf_oldzx1 / lf_oldzx1[0]) ** 2
                + (lf_tidzy0 / lf_tidzy0[0] - lf_oldzy0 / lf_oldzy0[0]) ** 2
                + (lf_tidzy1 / lf_tidzy1[0] - lf_oldzy1 / lf_oldzy1[0]) ** 2
            )
            return (
                lf_tih0,
                lf_tiv0,
    ...
    if nrdir == 3:
        ...
        if x1 == x2 and y1 == y2:
                ...
                if dif <= 3 * length:
                    if z1 < length:
                        ...
                        if z2 > sgd[2] - length:
                            ...
                            sgnewz = np.copy(sgoldz)
                            sgnewz[length], sgnewz[length + dif] = (
                                sgnewz[length + dif],
                                sgnewz[length],
                            )
                        else:
                    ...
                    sgoldz = np.append(
                        sgoldz0,
                        np.append(999 * np.ones(int(length), dtype=int), sgoldz1),
                    )
                    sgnewz = np.append(
                        sgnewz0,
                        np.append(999 * np.ones(int(length), dtype=int), sgnewz1),
                    )
            else:
            ...
            if z2 < length:
                ...
                sgnewz1[length] = nfase[0]
            sgoldz = np.append(
                sgoldz0, np.append(999 * np.ones(int(length), dtype=int), sgoldz1)
            )
            sgnewz = np.append(
                sgnewz0, np.append(999 * np.ones(int(length), dtype=int), sgnewz1)
            )
        return sgoldh, sgnewh, sgoldv, sgnewv, sgoldz, sgnewz
    elif nrdir == 6:

        sg_oldh, sg_newh, sg_oldv, sg_newv, sg_oldz, sg_newz = annstep3d(
            sg, x1, x2, y1, y2, z1, z2, sgd, 3, length, nfase
        )
        if z1 == z2:
            k1 = y1 - x1
            k2 = y2 - x2
            if k1 == k2:
                sg_newxy, sg_oldxy = ptdirup_3d2pts(
                    sg[:, :, z1], x1, y1, x2, y2, k1, nfase
                )
            else:
                sg_newxy0, sg_oldxy0 = ptdirup_3d(
                    sg[:, :, z1], x1, y1, nfase[1], length
                )
                sg_newxy1, sg_oldxy1 = ptdirup_3d(
                    sg[:, :, z2], x2, y2, nfase[0], length
                )
                sg_newxy = np.append(
                    sg_newxy0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newxy1),
        ...
        if y1 == y2:
            ...
            k2 = z2 - x2
            if k1 == k2:
                sg_newxz, sg_oldxz = ptdirup_3d2pts(
                    sg[:, y1, :], x1, z1, x2, z2, k1, nfase
                )
            else:
                sg_newxz0, sg_oldxz0 = ptdirup_3d(
                    sg[:, y1, :], x1, z1, nfase[1], length
                )
                sg_newxz1, sg_oldxz1 = ptdirup_3d(
                    sg[:, y2, :], x2, z2, nfase[0], length
                )
                sg_newxz = np.append(
                    sg_newxz0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newxz1),
                )
                sg_oldxz = np.append(
                    sg_oldxz0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldxz1),
                )
        else:
        ...
        if x1 == x2:
            ...
            k2 = z2 - y2
            if k1 == k2:
                sg_newyz, sg_oldyz = ptdirup_3d2pts(
                    sg[x1, :, :], y1, z1, y2, z2, k1, nfase
                )
            else:
                sg_newyz0, sg_oldyz0 = ptdirup_3d(
                    sg[x1, :, :], y1, z1, nfase[1], length
                )
                sg_newyz1, sg_oldyz1 = ptdirup_3d(
                    sg[x2, :, :], y2, z2, nfase[0], length
                )
                sg_newyz = np.append(
                    sg_newyz0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newyz1),
                )
                sg_oldyz = np.append(
                    sg_oldyz0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldyz1),
                )
        else:
        ...
        (
            ...
            sg_oldyz,
            sg_newyz,
        ) = annstep3d(sg, x1, x2, y1, y2, z1, z2, sgd, 6, length, nfase)
        ...
        if z1 == z2:
            ...
            k2 = y2 - (sgd[0] - 1 - x2)
            if k1 == k2:
                sg_newyx, sg_oldyx = ptdirup_3d2pts(
                    tempcopy_z, sgd[0] - 1 - x1, y1, sgd[0] - 1 - x2, y2, k1, nfase
                )
            else:
                sg_newyx0, sg_oldyx0 = ptdirup_3d(
                    tempcopy_z, sgd[0] - 1 - x1, y1, nfase[1], length
                )
                sg_newyx1, sg_oldyx1 = ptdirup_3d(
                    tempcopy_z, sgd[0] - 1 - x2, y2, nfase[0], length
                )
                sg_newyx = np.append(
                    sg_newyx0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newyx1),
                )
                sg_oldyx = np.append(
                    sg_oldyx0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldyx1),
                )
        else:
            tempcopy_z0 = sg[::-1, :, z1]
            tempcopy_z1 = sg[::-1, :, z2]
            sg_newyx0, sg_oldyx0 = ptdirup_3d(
                tempcopy_z0, sgd[0] - 1 - x1, y1, nfase[1], length
            )
            sg_newyx1, sg_oldyx1 = ptdirup_3d(
                tempcopy_z1, sgd[0] - 1 - x2, y2, nfase[0], length
            )
            sg_newyx = np.append(
                sg_newyx0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newyx1),
        ...
        if y1 == y2:
            ...
            k2 = z2 - (sgd[0] - 1 - x2)
            if k1 == k2:
                sg_newzx, sg_oldzx = ptdirup_3d2pts(
                    tempcopy_y, sgd[0] - 1 - x1, z1, sgd[0] - 1 - x2, z2, k1, nfase
                )
            else:
                sg_newzx0, sg_oldzx0 = ptdirup_3d(
                    tempcopy_y, sgd[0] - 1 - x1, z1, nfase[1], length
                )
                sg_newzx1, sg_oldzx1 = ptdirup_3d(
                    tempcopy_y, sgd[0] - 1 - x2, z2, nfase[0], length
                )
                sg_newzx = np.append(
                    sg_newzx0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newzx1),
                )
                sg_oldzx = np.append(
                    sg_oldzx0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldzx1),
                )
        else:
            tempcopy_y0 = sg[::-1, y1, :]
            tempcopy_y1 = sg[::-1, y1, :]
            sg_newzx0, sg_oldzx0 = ptdirup_3d(
                tempcopy_y0, sgd[0] - 1 - x1, z1, nfase[1], length
            )
            sg_newzx1, sg_oldzx1 = ptdirup_3d(
                tempcopy_y1, sgd[0] - 1 - x2, z2, nfase[0], length
            )
            sg_newzx = np.append(
                sg_newzx0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newzx1),
        ...
        if x1 == x2:
            ...
            if k1 == k2:
                sg_newzy, sg_oldzy = ptdirup_3d2pts(
                    tempcopy_x, sgd[1] - 1 - y1, z1, sgd[1] - 1 - y2, z2, k1, nfase
                )
            else:
                sg_newzy0, sg_oldzy0 = ptdirup_3d(
                    tempcopy_x, sgd[1] - 1 - y1, z1, nfase[1], length
                )
                sg_newzy1, sg_oldzy1 = ptdirup_3d(
                    tempcopy_x, sgd[1] - 1 - y2, z2, nfase[0], length
                )
                sg_newzy = np.append(
                    sg_newzy0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newzy1),
                )
                sg_oldzy = np.append(
                    sg_oldzy0,
                    np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_oldzy1),
                )
        else:
            tempcopy_x0 = sg[x1, ::-1, :]
            tempcopy_x1 = sg[x2, ::-1, :]
            sg_newzy0, sg_oldzy0 = ptdirup_3d(
                tempcopy_x0, sgd[1] - 1 - y1, z1, nfase[1], length
            )
            sg_newzy1, sg_oldzy1 = ptdirup_3d(
                tempcopy_x1, sgd[1] - 1 - y2, z2, nfase[0], length
            )
            sg_newzy = np.append(
                sg_newzy0,
                np.append(999 * np.ones(int(length / 1.414), dtype=int), sg_newzy1),
